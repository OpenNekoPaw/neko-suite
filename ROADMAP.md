...
| **neko-agent** | Alpha | 99% | **0 TODOs**, 1957+ tests, 300+ files; 7 LLM + 10 media adapters + MCP + Coordinator + SubAgent + Creative Memory + quality assessment + Webview P0 complete + **SDD Unified Workflow Phase A+B+closure ✅** (3-stage draft/plan/apply, ArtifactValidator/Watcher/ObservationHooks closed-loop self-correct, §4 revision 2026-04-22); remaining: MCP reconnection backoff + P1 Zustand migration + ask-mode decoupling ([ADR](./docs/architecture/agent-unified-workflow.md)) |
...
### neko-agent — SDD Unified Workflow (Phase A + B + B-closure ✅, 2026-04-22)
...
- [ ] **P1 — `ask` mode decoupling**: `ExecutionMode 'ask'` (per-tool confirmation UX) and `StageMode 'ask'` (SDD planner mode) currently share the same string via cast; redesign the permission/SDD boundary so stage planning only knows `plan`/`auto`.
...
- [ ] **P3 — 154 pre-existing TS errors**: `MCPTool`/`BashTool` parameters mismatches + `ToolParameters` shape drift; independent of the SDD refactor.
- [ ] **P3 — 5 pre-existing `fileOperationHandler.test.ts` failures**: vscode mock divergence; unrelated to the SDD refactor.
