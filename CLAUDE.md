...
| AI Face Sculpting | [docs/architecture/adr-ai-face-sculpting.md](./docs/architecture/adr-ai-face-sculpting.md) | **Proposed (2026-05-07)**. Agent 捏脸三阶段演进：开环生成（P0, 复用 puppet 模式到 model）→ 闭环迭代（P1, Engine 截图端点 + 视觉验证 prompt）→ 多模态联合编辑（远期 MCP 桥接）。阶段 1-2 不需要专业模型；通用 LLM + 结构化 prompt + 参数钳制已被 puppet 验证。neko-model 补 AgentCapabilityProvider（3 工具）+ NekoModelAPI face params 接口。 |
