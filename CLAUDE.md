...
| Dual-Flow Architecture | [docs/architecture/dual-flow-architecture.md](./docs/architecture/dual-flow-architecture.md) | Creation Flow (Orchestration→Proposal→Review→Execution→Status) + Execution Flow (Plan→TODO→Approve→Apply→Step) dual-language system. Single Agent + dual Skills. Single Approval Engine + two strategy packs. 5-level autoheal (80%+ technical issues self-resolve). Proposed |
