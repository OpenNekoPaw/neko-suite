...
| NPC Character Test Bench | [docs/architecture/adr-npc-character-test-bench.md](./docs/architecture/adr-npc-character-test-bench.md) | **Proposed (2026-05-29)**. `/as @entity` 触发 NPC 测试 SubAgent：隔离 session（无创作工具、封闭知识边界）；两阶段 profile 聚合（确定性实体数据 + 可选 AI 剧本提取）；NpcProfileProjector 纯投影函数（不新建 CharacterCard 类型）；`npc-character` SubAgent preset；Dashboard `test-npc` action 快速入口；退出后生成 NPC 评估报告（人格一致性/知识泄漏/关系空白）反馈到实体编辑循环。~8d 9 PR。 |
| Headshot 3 Capability Gap | [docs/architecture/adr-headshot3-capability-gap-analysis.md](./docs/architecture/adr-headshot3-capability-gap-analysis.md) | **Proposed (2026-05-30)**. Headshot 3 对标分析：13 项 HS3 功能 vs neko-model API+管线组合架构。管线已覆盖（text-to-image/超分/ControlNet）；管线可达（照片→3D/De-light/法线/体型，需 ReplicateProcessingAdapter）；真正差距（样条曲线塑形 UI/3D 纹理投射管线/Morph 资产规模/retopology）。四优先级：Replicate Adapter→API→VLM 闭环→纹理投射→Morph 扩充。 |
| Model LookDev Scene Editing | [docs/architecture/adr-model-lookdev-scene-editing.md](./docs/architecture/adr-model-lookdev-scene-editing.md) | **Proposed (2026-05-30)**. LookDev（Clay/Wireframe/Normal/Depth 8 模式）+ 灯光 CRUD + 环境 IBL/背景 + 语义部位选择。Engine 权威 + Webview 控制面；命令信封 seq/correlationId/baseRevision；P0-P5 六阶段；选择 mask 6 工作流模式。 |
| Engine vs Bevy Gap Analysis | [docs/architecture/adr-engine-bevy-gap-analysis.md](./docs/architecture/adr-engine-bevy-gap-analysis.md) | **Proposed (2026-06-04)**. 决策：不集成完整 Bevy（架构不匹配 sidecar headless）。57 crate 三分法：不需要 17 / 可集成 22 / 需自研 12。可直接依赖：bevy_tasks+bevy_color+bevy_math。Shader 移植 11 项 ~3360 行（shadow/SSAO/IBL/morph/TAA/DOF/blur/ray/gizmo/mipmap/bloom）~12 周。P0：shadow+picking+gizmo（3D 编辑器最低门槛）~5 周。设计参考：AnimationGraph/AssetServer/Material trait/增量传播。 |
| Canvas Interactive Narrative | [docs/architecture/adr-canvas-interactive-narrative.md](./docs/architecture/adr-canvas-interactive-narrative.md) | **Proposed (2026-06-07, revised 2026-06-08)**. 线性/分支分治：线性剧情以 neko-story 为主，多分支剧情以 Canvas narrative 子系统为创作核心（`.nkc` 即故事图 SSOT，不引入 `.nkstory` 中间格式）。编辑+预览分离：Canvas 纯图编辑 + 独立 Narrative Preview Webview。新增 `narrative-start` / `narrative-ending` 节点类型。人物演绎分级（L0 立绘→L1 CSS→L2 Live2D→L3 Spine→L4 3D）。三种 PlayRenderer 按 genre 分派。Preview = 导出原型。~15d 10 PR + P3 增强 4d 2 PR。 |
