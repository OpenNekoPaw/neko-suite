...
| Agent Unified Workflow | [docs/architecture/agent-unified-workflow.md](./docs/architecture/agent-unified-workflow.md) | SDD 3-stage workflow (Draft → Plan → Apply, 2026-04-22 revision) + four-layer architecture (L3 Mode / L2 Flow / L1 Capability / L0 Infra). Artifacts use `<kind>-<runId>.md` prefix convention under `.neko/drafts\|plans\|tasks/`. Phase B removed DraftWrite/PlanWrite/TaskWrite in favour of generic Write + ArtifactWatcher (post-write frontmatter validation). Supersedes dual-flow-architecture.md |
