...
| Canvas Interactive Narrative | [docs/architecture/adr-canvas-interactive-narrative.md](./docs/architecture/adr-canvas-interactive-narrative.md) | **Proposed (2026-06-07)**. Canvas 双模设计（Graph Mode 图编辑 + Play Mode 沉浸预览）支持互动影游/视觉小说/图文游戏三类交互叙事创作。Story-centric SSOT：`.nkstory` 场景图 JSON + 标准 Fountain 片段（零格式发明）。三种 PlayRenderer（InteractiveFilm/VisualNovel/IllustratedText）按 genre 分派。NarrativeRuntime 状态机。StoryProjectionAdapter 复用 Multi-Purpose Canvas 投影机制。HTML5/Electron 导出。~20d 13 PR。 |
