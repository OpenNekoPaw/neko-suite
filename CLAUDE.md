...
| Engine Interface & Pipeline Decoupling | [docs/architecture/adr-engine-interface-pipeline-decoupling.md](./docs/architecture/adr-engine-interface-pipeline-decoupling.md) | **Proposed (2026-05-12)**. 引擎全面解耦审计（9 Part）。Part I: IPC/WS 分离 + 17 controller + EditOperation 三级优先级 + 工程文件支持。Part II: 三条 GPU 零拷贝热路径（Timeline/Video/Export）+ GpuExportPipeline 输出无关核心 + PreviewPipeline 编码焊接。Part III: FrameSink trait（BroadcastSink/MuxerSink/SnapshotSink）+ ProducedFrame::GpuHandle 保留零拷贝。Part IV: GPU 资源预算控制器（Interactive>Export>Transcode + 降级策略）。Part V: GpuEffect trait + AudioEffectFactory + EffectRegistry + Plugin 桥接 + ML 三阶段集成。Part VIII: Puppet/Scene 计算-渲染-IO 三层分离（SceneService 拆分 + PuppetRenderer wgpu SpriteBatch + Puppet WebSocket 命令协议 + 三层架构 Input→Computation→Render→FrameSink）。Part IX: 双 API 模型——CreativeAccess（OOP 用户意图抽象，有版本控制+undo）+ DataAccess（ECS 数据操作抽象，无版本控制）；消除 SceneService 18 处 `ecs_world_mut()` 逃逸；调用者权限矩阵（Controller→Creative only / Renderer→Data read / DomainSystem→both）。P0~P3 实施计划（17 PRs）。 |
