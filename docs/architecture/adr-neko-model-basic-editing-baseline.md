...
| 层级 | 能力 | 用户可感知验收 | 优先级 |
|---|---|---|---|
| B0: 可视化 | 加载模型、显示 Engine stream、相机 orbit/pan/zoom、默认目标 1080p/60fps 清晰显示 | 用户能稳定看到 mesh，旋转观察不卡顿，PBR/白模/线框至少可切换；若只能降到 720p，必须是显式 fallback 并显示原因 | P0 |
| B1: 对象级编辑 | 选中 node/submesh/material slot，Transform 平移/旋转/缩放，显示 Outliner/Inspector | 点击模型或树节点后 Inspector 可编辑，并实时反映到画面 | P0 |
| B2: 场景 LookDev | PBR/白模/线框/法线/深度切换，添加/删除/移动灯光，背景色/环境切换 | 按钮不灰，操作后 Engine 画面变化，失败有明确原因 | P0 |
...
| 维度 | 设计结论 |
|---|---|
| 职责 | Webview 只做控制 UI、状态展示和 overlay；Engine 负责模型加载、场景状态、渲染、拾取和编辑命令应用。 |
| 依赖 | 基础编辑依赖 Route A scene control、SceneSnapshot/SceneDelta、ViewportCommand/SceneCommand；不依赖 AI provider。 |
| 接口 | 所有写操作必须走 command envelope；所有按钮启用状态必须来自能力声明、场景快照和选中上下文，而不是硬编码 UI 假设。 |
| 扩展 | 普通 mesh 从 object/material/submesh 开始；角色资产再扩展 morph/bone/region；Headshot/AI 输出必须先归一化到角色资产合同。 |
| 测试 | 必须增加端到端 smoke：加载固定 GLB 后对象选择、Transform、LookDev 切换、灯光 CRUD、背景色切换均可操作；无能力时按钮显示具体原因。当前可用 `../neko-test/test.glb` 做手工验证，但 CI 需要仓库内新增可再分发 fixture。 |
...
| E3 | Transform 编辑闭环 | E2 | 1 PR / 2-3 天 | 平移/旋转/缩放写入 SceneCommand，SceneDelta 回显，失败可回滚 |
...
任一按钮不可用时，用户可以看到原因。原因必须来自 Engine capability、scene control status、scene snapshot 或当前 selection context，而不是 UI 硬编码猜测。
