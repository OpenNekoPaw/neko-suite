...
| `neko-dashboard` | 5 | 271 | Dashboard UI 组件候选清理 |
...
| `neko-agent` | 4 | 68 | 小 helper 文件候选清理 |
| `test-fixtures` | 1 | 17 | fixture 需确认是否保留给外部调试 |
...
| 包 | 主要问题 |
|----|----------|
| `neko-agent` | 未使用 deps：`adm-zip`、`epub2`、`fast-xml-parser`、`node-fetch`、`node-unrar-js`、`xlsx`；未声明 deps：`@neko/ai-sdk`、`undici`、`@testing-library/react`、`@neko/shared`、`@neko-agent/types` 等；另有大量未使用导出 |
| `neko-canvas` | 多个 barrel / node-card / connection helper export 未被本仓库引用 |
| `neko-sketch` | tool / shader / store helper export 未被引用 |
| `neko-puppet` | `jsdom` 未声明，部分 webview animation export 未引用 |
| `neko-dashboard` | project type / i18n / render guard export 未引用 |
| `neko-engine` | scripts export 静态图未引用 |
...
非测试源码命中 `deprecated / legacy / obsolete` 共 373 处：

| 包 | 命中 | 文件数 | 主要性质 |
|----|------|--------|----------|
| `neko-types` | 148 | 38 | 旧 schema 字段、migrator、shared UI compatibility surface、配置迁移 |
| `neko-agent` | 101 | 32 | legacy adapter、旧工具路径、compat shim、workflow deprecation metadata |
| `neko-engine` | 52 | 27 | INP 兼容、legacy CPU / encode-only pipeline、deprecated Rust API |
| `neko-canvas` | 13 | 6 | legacy anchor / container 兼容 |
| `neko-market` | 12 | 8 | marketplace package deprecation 状态与 API |
| `neko-puppet` | 8 | 5 | INP read-only / backward-compatible aliases |
| `neko-entity` | 8 | 2 | CreativeEntity deprecated 状态 |
| `neko-cut` | 7 | 4 | binary/base64 fallback、旧 transition 字段 |
| `neko-assets` | 6 | 2 | `legacyFallbackRef` metadata |
| 其他包 | 18 | 14 | 状态字段、文档消息 fallback、proto 兼容 |
...
`TODO(P*) / FIXME / HACK` 共 28 处：

| 包 | 命中 | 主要内容 |
|----|------|----------|
| `neko-agent` | 11 | AgentRunnerPort fallback、built-in skill 工具缺口、projectSearch compat shim |
| `neko-engine` | 8 | preview provider / encode-only backend / legacy CPU pipeline / source model tracking |
| `neko-story` | 7 | 文档计划中的 CreativeEntityGraph / OccurrenceIndex 后续项 |
| `neko-live` | 1 | TrackingService ownership 迁移 |
| `neko-market` | 1 | patch format 应用 |
...
| 包 | 候选 |
|----|------|
| `neko-dashboard` | `ContextStrip.tsx`、`ProjectTable.tsx`、`QuickActions.tsx`、`RecentActivity.tsx`、webview `logger.ts` |
| `neko-canvas` | `InlineControls.tsx`、`ImageViewer.tsx`、`components/content/index.ts` |
| `neko-sketch` | `gradient-shaders.ts`、`psd-import.ts` |
| `neko-agent` | `message-helpers.ts`、`media-extractors.ts`、`tool-constants.ts` |
| `neko-story` | unused webview `tailwind.config.js` |

依赖清理：

1. 移除确认为 unused 的 package deps / devDeps。
2. 补齐实际使用但未声明的 deps，优先放到实际 workspace package，而不是根包。
3. 对测试依赖如 `jsdom`、`@testing-library/react`，统一到对应 package devDependencies 或 workspace 约定。
...
| id | 包 | 入口 | 分类 | 替代/删除条件 |
|----|----|------|------|---------------|
| LCD-001 | `neko-agent` | `packages/extension/src/services/projectSearch/*` | migration-adapter | internal imports 全部迁到 `@neko/search` 后删除 |
| LCD-002 | `neko-agent` | legacy tool registration path / `puppetFaceTools.ts` | stray-surface | per-package CapabilityProvider 完全覆盖并通过 duplicate guard |
| LCD-003 | `neko-types` | `src/components/index.ts` | stray-surface | Agent-safe file-drop / icon strategy 完成后删除 React UI legacy exports |
| LCD-004 | `neko-engine` | legacy CPU / encode-only preview pipeline | migration-adapter | Engine pipeline sink / preview provider 完成后删除 |
| LCD-005 | `neko-puppet` / `neko-engine` | INP metadata-only support | canonical compatibility | 明确不再支持旧文件打开或完成迁移工具后删除 |
| LCD-006 | `neko-canvas` | legacy anchors / group childIds | canonical compatibility | `.nkc` schema migration 和 fixtures 确认后删除 fallback |
