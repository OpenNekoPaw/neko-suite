...
### Workflow-Native Only

旧 pipeline / prompt-chain compatibility adapter 不再作为流程入口扩展。prelaunch 阶段没有上线兼容窗口，runtime 不再保留 legacy workflow telemetry recorder 或 sunset gate；多步骤 Agent 流程必须直接提供 `AgentWorkflowDefinition`、workflow node profile 和 canonical IDC stage state。

旧 DSL 或 pipeline-only 输入如需恢复，只能在输入边界一次性迁移成 canonical workflow definition；runtime 内部不接受分散的 `legacy step -> workflow node` 兜底映射。
