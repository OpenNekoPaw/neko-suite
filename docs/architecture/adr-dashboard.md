...
`creativeEntityAction` messages follow the same host-mediated rule: the Webview sends only a shared `DashboardCreativeEntityActionRequest`, the Extension Host validates the source/ref/action shape, and the owning source performs or delegates the domain operation. Dashboard never writes Story registries, entity binding files, requirement files, visual draft files, character memory ledgers, or Assets metadata directly. Source navigation such as `open-source` is also source-owned: the source resolves registry/script locations in Extension Host code and opens the editor there, while Webview state keeps only safe projected refs.

Character memory review uses the same source-owned path. A creative entity detail MAY expose `memoryReviews` as projected review rows for draft, needs-review, or conflict observations. The Dashboard Webview renders those rows and sends `accept-memory-review`, `reject-memory-review`, `mark-memory-conflict`, or `supersede-memory-review` with `memoryReviewId`; the owning `DashboardCreativeEntitySource` loads and updates the `CharacterEvidenceLedger` through its host-side store. The neutral `neko-entity` source is the first implementation: it projects reviewable observations from the project character-memory ledger and writes accepted/rejected/conflict/superseded status through the shared character-memory operations. Other sources can add the same optional projection without changing Dashboard core.
