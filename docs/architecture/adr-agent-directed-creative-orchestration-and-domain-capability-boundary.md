...
不得新增 `WorkflowRuntime`、`WorkflowRun`、`WorkflowNode`、DAG scheduler、固定 stage executor、Creative Observation Store、独立 Plan Manager 或平行 Creation 状态机。也不增加单独的“开始编排”按钮或编排页面：用户通过现有对话提出目标、补充信息、审批和继续下一步。
...
Prompt-chain 仍是可跳过、重排和重复的方法指导，不是 Workflow DSL。`started`、`checkpoint`、`skipped`、`reordered`、`completed` 等 observation 如保留，只用于解释或评估，不是 Agent 继续执行所必需的状态，也不能证明文件、镜头或项目已经完成。
...
用户通过同一对话阅读、修改和批准这些文档；不增加 IDC、Plan 或编排按钮。高成本、高风险或需要确定性授权的批准可以绑定文档 content digest、关键输入、目标、创作范围、成本/风险上限、mutation 和交付范围。
...
若用户显式批准一份高成本或高风险方案，Approval 可以绑定该 `plan.md` 的 content digest 和明确批准范围。普通对话计划、TODO 和分析文档不需要全局 revision 管理。
...
| 已批准的 `plan.md` | 仅在批准后绑定 plan digest 和批准范围 | 不形成项目 revision |
| 对话、TODO、分析 Markdown、prompt-chain observation | 临时上下文、说明或审阅记录 | 否 |

生成资产就是用户可直接使用的文件，不需要先晋升成另一种“稳定 Artifact”。Asset Library 的 Import/Promote 只在用户明确需要正式入库、实体绑定或项目复用时执行。

Agent 不维护全局 current revision，也不递归扫描历史消息寻找“最新 revision”。Tool 结果应直接返回下一步所需的文件、ResourceRef、digest、lineage、项目 revision 或 diagnostic；对 `.nk*` consequential mutation，由 owning runtime 在写入前校验 `baseRevision`。这比通用 Revision Observation Store 更直接，也避免形成第二份项目事实。
...
领域包通过共享 contract、Tool contribution、ResourceRef 和项目 authoring result 协作，不直接导入另一个功能包的内部实现。需要多个低级 mutation 才能保证一致性的操作，由 owning package 提供小型事务性 headless Tool，并返回 exact project revision；Agent 不直接拼装 `.nk*` 私有格式，也不依赖 active Webview 猜测目标。
...
`auto`、`ask`、`plan` execution modes 保留：Plan Mode 负责只读分析和审批前计划，Approval 负责副作用授权，领域 Skill 负责专业方法，普通 Agent ReAct 负责下一步判断和执行。通用 Approval、preferences、logging、autoheal、validation、document IO、Task 和异步续作必须从 `stageTracking` 解耦，不能随 IDC 删除而丢失。
...
1. 将 Approval、preferences、logging、autoheal、validation、document IO、Task 和续作从 `stageTracking` 解耦，再删除固定 IDC types/runtime/personas/artifacts；
2. 把 Plan Mode 改为领域无关、只读但具体的计划模式，使其能够分析真实内容并生成 execution-ready work units；
3. 复用现有 conversation/task surface 提供轻量 TODO 投影，不创建新的进度状态 owner；
4. 改造影视动画 Skill，使其指导来源证据分析、创作者审批文档、条件判断、技术选择、执行工作单元、跳过/重排和恢复，而不是固定步骤或总结模板；
5. 审计现有 Tool description/schema/result，确保 Agent 能理解真实输入、输出、执行类型、限制和 diagnostic；
6. 补齐漫画分析与准备、角色参考、分镜 authoring、镜头技术、Cut Animatic、Audio、Quality 和 Export 所缺的 owning-package 原子 Tool；
7. 修复异步 Task 完成后的原对话唤醒和 Agent 自主续作；
8. 用真实 Agent evaluation 证明 Agent 无 IDC 也能生成可审批文档、具体计划、连续调用真实 Tool、根据失败换技术并产生可直接使用的文件；
9. 只有 evaluation 证明有必要时，再从现有 registry 渐进增加紧凑能力查询，不预先建设通用 planning runtime。

## 禁止路径

- 禁止建立漫画到动画、小说到电影等固定隐藏 Pipeline 或 Workflow DAG。
- 禁止新增或保留与现有 Agent 平行的 IDC/stage、创作 runtime、Plan Manager、TodoManager、Observation Store 或操作入口。
