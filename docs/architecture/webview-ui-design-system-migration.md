...
| Cut Webview | Cut package | `Timeline/TimelineRuler.tsx`, `hooks/useTimelineContextMenu.ts`, `components/ContextMenu.tsx`, `components/PropertyPanel/PropertyPanel.tsx` | Timeline ruler, AI menu, context menu, and outer collapsible shell are outside the 7.x core PropertyPanel row migration; resize/drag hooks have moved to `@neko/ui` | Phase 4 cleanup after shared timeline/menu/collapsible replacements are accepted |
| Puppet Webview | Puppet package | `PuppetKeyframeTimeline.tsx` | Layout resize imports have moved to `@neko/ui`; keyframe timeline remains outside 8.x parameter/tree migration | Phase 4 cleanup after shared timeline contract lands in `@neko/ui` |
| Model Webview | Model package | `ModelKeyframeTimeline.tsx`, `Toolbar.tsx` | Layout resize imports have moved to `@neko/ui`; keyframe timeline and toolbar primitives were not part of the 8.x transform/scene-tree slice | Phase 4 cleanup after toolbar/timeline migration |
| Sketch Webview | Sketch package | `CollapsiblePanel.tsx`, `Toolbar.tsx`, `SketchCanvas.tsx` | Layout resize imports have moved to `@neko/ui`; generic collapsible shell, toolbar, and canvas context menu are outside the 9.x BrushPanel/LayerPanel migration | Phase 4 cleanup after toolbar/canvas menu migration |
| Canvas Webview | Canvas package | `components/common/ContextMenu.tsx`, `components/media/InlineAudioPlayer.tsx`, `components/media/InlineVideoPlayer.tsx`, `components/toolbar/CanvasToolbar.tsx` | Node gesture and file-drop hooks have moved to `@neko/ui`; AI context menu, media players, and toolbar remain outside the 9.x PropertyPanel/NodeLibrary migration | Later Canvas/menu/media wave or Phase 4 cleanup |
| Audio Webview | Audio package | `Toolbar.tsx`, `Timeline/AudioClip.tsx`, `Timeline/TimelineRuler.tsx`, `Timeline/TrackLane.tsx`, `EditableWaveform.tsx` | Transport, effects, export, recording, preset browser, side panel, and drag/drop now use `@neko/ui`; remaining exemptions are timeline/menu/toolbar surfaces outside the 11.1 low-risk control slice | Phase 4 timeline/menu/toolbar cleanup |
...
| Audio | DAW panel/timeline aliases | Transport/effects/export/recording controls now consume shared `Button`, `IconButton`, `Select`, `Slider`, and `@neko/ui/icons`; timeline and toolbar aliases remain package-owned | 11.1 later wave controls | Phase 4 timeline/menu audit |
...
| Animation controls | `AnimationPanel.tsx` and `PuppetKeyframeTimeline.tsx`; timeline still imports `KeyframeTimeline` from `@neko/shared/components` | Treat playback controls separately from parameter migration; keyframe timeline legacy import stays exempt until shared timeline is in `@neko/ui` |
...
| Toolbar controls | `Toolbar.tsx` and `VectorToolbar.tsx` still contain larger tool-specific local icon/tool behavior | Not migrated in 9.x except related LayerPanel header actions; toolbar full icon convergence remains a later package wave or Phase 4 audit item |
...
| Toolbar controls | `CanvasToolbar.tsx` and `CanvasTopToolbar.tsx` still contain local toolbar behavior, some inline SVG, and local select controls | Not migrated in this 9.x slice; toolbar migration remains a later wave because it touches interaction mode, auto-arrange, playback, and settings control flow |
| Context menus | `components/common/ContextMenu.tsx` still wraps package-specific Canvas node/edge menus and AI actions | Not migrated in 9.x; menu migration requires preserving AI menu sections and node/edge action semantics |
...
Canvas token/icon note: touched PropertyPanel/NodeLibraryPanel files no longer import `@neko/shared/components` or `@neko/shared/icons`, and migrated lock/delete controls no longer use emoji labels. Remaining inline SVG and emoji controls in Canvas node renderers, common context menus, media viewers, and toolbars are outside this touched slice and remain Phase 4/later-wave cleanup candidates.
...
|---------|------------------|---------------------------------|--------------|
| Audio | `components/shared/AudioUiPrimitives.tsx` adapts `Button`, `IconButton`, `Select`, and `Slider`; transport, effects, export, recording, preset browser, side panel, empty project, and drag/drop now consume `@neko/ui` or `@neko/ui/icons` | DAW toolbar, timeline context menus, `TimelineRuler`, and `EditableWaveform` remain exempt because they need timeline/menu contracts rather than primitive-only replacement | `pnpm --filter @neko-audio/webview exec tsc --noEmit` |
