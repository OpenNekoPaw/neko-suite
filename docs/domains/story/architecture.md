...
| 参与者               | 职责                                                         |
| -------------------- | ------------------------------------------------------------ |
| `neko-story`         | 剧本文本、parser、LSP、场景索引、readiness 和规划命令入口     |
| `neko-canvas`        | 消费 storyboard payload，承接场景级节点、候选图和执行摘要     |
...
- 剧本文本、AST、sceneId 和 readiness 聚合由 `neko-story` 拥有。
- Canvas 只接收结构化 storyboard payload 和只读执行摘要，不成为第二份剧本数据源。
...
script text
  -> parser / script index
  -> scene plan / shot plan / readiness
  -> Agent suggestions or storyboard payload
  -> canvas nodes / video timeline / asset requirements
```
