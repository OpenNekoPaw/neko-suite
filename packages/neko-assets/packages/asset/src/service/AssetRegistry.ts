...
const MEDIA_TYPES: ReadonlySet<AssetType> = new Set([
...
export class AssetRegistry implements IAssetRegistry {
	...
	async register(manifest: AssetManifest): Promise<string> {
		...
		if (handler && manifest.source.kind === 'local') {
			...
			if (!validation.valid) {
				throw new Error(
					`Validation failed for ${manifest.type}: ${validation.errors?.join(', ')}`
				);
			}
		}
	...
	async resolve(id: string): Promise<string | undefined> {
		...
		const entity = await this.library.getEntity(id);
		if (entity) {
			const defaultVariant = entity.variants.find(
				v => v.id === entity.defaultVariantId
			) ?? entity.variants[0];
	...
	private matchesFilter(manifest: AssetManifest, filter: AssetRegistryQuery): boolean {
		...
		if (filter.text) {
			...
			const tagMatch = manifest.distribution?.tags?.some(
				t => t.toLowerCase().includes(searchText)
			);
