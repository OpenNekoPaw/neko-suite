...
export class JsonFileStorage implements IAssetStorageWithEvents {
	...
	async saveFile(variantId: string, file: AssetFile): Promise<void> {
		for (const entity of this.entities.values()) {
			const variantIndex = entity.variants.findIndex(
				(v) => v.id === variantId
			);
			if (variantIndex !== -1) {
	...
	async search(query: AssetQuery): Promise<SearchResult> {
		...
		if (query.anyTags && query.anyTags.length > 0) {
			results = results.filter((e) =>
				query.anyTags!.some((tag) => e.tags.includes(tag))
			);
		}
		...
		if (query.sourceTypes && query.sourceTypes.length > 0) {
			results = results.filter(
				(e) =>
					e.metadata.source?.type &&
					query.sourceTypes!.includes(e.metadata.source.type)
			);
		}
		...
		if (query.usedAfter) {
			results = results.filter(
				(e) => e.lastUsedAt && e.lastUsedAt >= query.usedAfter!
			);
		}
		if (query.usedBefore) {
			results = results.filter(
				(e) => e.lastUsedAt && e.lastUsedAt <= query.usedBefore!
			);
		}
