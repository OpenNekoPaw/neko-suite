...
describe('FileService', () => {
	...
	describe('add', () => {
		it('should add a file to a variant', async () => {
			const file = await fileService.add(
				testVariant.id,
				'/assets/character/hero_front.png'
			);
		...
		it('should throw error for non-existent variant', async () => {
			await expect(
				fileService.add('non-existent', '/path/to/file.png')
	...
	describe('get', () => {
		it('should retrieve an existing file', async () => {
			const created = await fileService.add(
				testVariant.id,
				'/assets/test.png'
			);
	...
	describe('updateMetadata', () => {
		it('should update file metadata', async () => {
			...
			const updated = await fileService.updateMetadata(
				testVariant.id,
				file.id,
				{
					duration: 120,
