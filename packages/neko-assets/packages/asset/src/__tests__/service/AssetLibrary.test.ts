...
describe('AssetLibrary', () => {
	...
	describe('AI classification', () => {
		...
		it('should use classifier when available', async () => {
			...
			const classification = await libraryWithClassifier.classifyFile(
				'/assets/test.png'
			);
			expect(classification?.suggestedCategory).toBe('character');
			expect(classification?.suggestedName).toBe('Hero');

			const similar = await libraryWithClassifier.findSimilarEntities(
				'/assets/test.png'
			);
		...
		it('should auto-classify on import when requested', async () => {
			...
			const result = await libraryWithClassifier.importFile(
				'/assets/fire.png',
				{ autoClassify: true }
			);
