...
describe('PathResolver', () => {
	...
	describe('resolve', () => {
		it('should expand variable to absolute path', () => {
			expect(resolver.resolve('${TEAM_FOOTAGE}/scene01/clip.mp4'))
				.toBe('/Volumes/NAS/footage/scene01/clip.mp4');
		});

		it('should handle variable-only path', () => {
			expect(resolver.resolve('${TEAM_FOOTAGE}'))
				.toBe('/Volumes/NAS/footage');
		});

		it('should return as-is if no variable pattern', () => {
			expect(resolver.resolve('/absolute/path/file.mp4'))
				.toBe('/absolute/path/file.mp4');
		});

		it('should return as-is if variable not defined', () => {
			expect(resolver.resolve('${UNKNOWN_VAR}/file.mp4'))
				.toBe('${UNKNOWN_VAR}/file.mp4');
		});

		it('should handle deeply nested paths', () => {
			expect(resolver.resolve('${PERSONAL_SFX}/impacts/heavy/boom.wav'))
				.toBe('/Users/user/SFX/impacts/heavy/boom.wav');
		});

		it('should not produce double slashes', () => {
			resolver.setVariables(new Map([
				['WITH_SLASH', '/path/with/trailing/'],
			]));
			expect(resolver.resolve('${WITH_SLASH}/file.mp4'))
				.toBe('/path/with/trailing/file.mp4');
		});
	});

	describe('contract', () => {
		it('should contract absolute path to variable path', () => {
			expect(resolver.contract('/Volumes/NAS/footage/scene01/clip.mp4'))
				.toBe('${TEAM_FOOTAGE}/scene01/clip.mp4');
		});
		...
		it('should pick longest matching variable (most specific)', () => {
			...
			expect(resolver.contract('/Volumes/NAS/footage/premium/clip.mp4'))
				.toBe('${NESTED_LIB}/clip.mp4');
		});

		it('should return as-is if no variable matches', () => {
			expect(resolver.contract('/other/path/file.mp4'))
				.toBe('/other/path/file.mp4');
		});

		it('should handle exact match (path equals variable base)', () => {
			expect(resolver.contract('/Users/user/SFX'))
				.toBe('${PERSONAL_SFX}');
		});
