...
import { COMPATIBLE_MODE_CAPABILITIES } from '@uniedit/shared';
...
// MediaProcessor type from @vedit/media-processor-rs
...
export class NativeMediaEngine implements IMediaEngine {
	...
	private async loadNativeProcessor(): Promise<void> {
		...
		try {
			...
			const module = await import('@vedit/media-processor-rs') as MediaProcessorModule;
...
class NativeVideoDecoder implements IDecoder {
	...
	async decodeNext(): Promise<import('@uniedit/shared').DecodedVideoFrame | null> {
		return this.decodeAt(this._position);
	}

	async decodeAt(time: number): Promise<import('@uniedit/shared').DecodedVideoFrame | null> {
		this._position = time;
	...
	private async decodeWithFFmpegService(time: number): Promise<import('@uniedit/shared').DecodedVideoFrame | null> {
	...
	async *decodeRange(
		...
		duration: number,
		fps: number
	): AsyncGenerator<import('@uniedit/shared').DecodedVideoFrame, void, undefined> {
...
class NativeAudioDecoder implements IDecoder {
	...
	async decodeNext(): Promise<import('@uniedit/shared').DecodedAudioFrame | null> {
	...
	async decodeAt(time: number): Promise<import('@uniedit/shared').DecodedAudioFrame | null> {
	...
	async *decodeRange(
		startTime: number,
		duration: number
	): AsyncGenerator<import('@uniedit/shared').DecodedAudioFrame, void, undefined> {
...
class NativeEncoder implements IEncoder {
	private _state: import('@uniedit/shared').EncoderState = 'idle';
	...
	// Event listeners
	private _progressListeners: Set<(progress: import('@uniedit/shared').EncoderProgress) => void> = new Set();
	private _stateListeners: Set<(state: import('@uniedit/shared').EncoderState) => void> = new Set();
	...
	async finalize(): Promise<import('@uniedit/shared').EncoderResult> {
		try {
	...
	get onProgress(): import('@uniedit/shared').EncoderEvent<import('@uniedit/shared').EncoderProgress> {
		return (listener) => {
	...
	get onStateChange(): import('@uniedit/shared').EncoderEvent<import('@uniedit/shared').EncoderState> {
		return (listener) => {
			this._stateListeners.add(listener);
	...
	get onError(): import('@uniedit/shared').EncoderEvent<Error> {
		return (listener) => {
			this._errorListeners.add(listener);
