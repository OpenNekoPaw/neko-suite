...
- **依赖**: `@uniedit/effects-core` (类型), `@uniedit/shared` (接口), `@vedit/media-processor-rs` (wgpu 后端)
...
```typescript
import { createWgpuEffectRunner } from '@uniedit/effects-runtime';
...
```typescript
import { createWebGPUEffectRunner } from '@uniedit/effects-runtime';
