...
import type {
  DesktopAgentSkillsSnapshot,
  DesktopAgentSnapshotRuntime,
} from './agent-webview-host';

export interface DesktopSkillFileSnapshotRuntimeOptions {
  readonly workspaceRoot: string;
  readonly homeDir?: string;
  readonly fs?: DesktopSkillCatalogFs;
  readonly path?: DesktopSkillCatalogPath;
}

export interface DesktopSkillCatalogFs {
  readdir(
    dirPath: string,
    options: { readonly withFileTypes: true },
  ): Promise<readonly DesktopSkillCatalogDirent[]>;
  readFile(filePath: string, encoding: 'utf-8'): Promise<string>;
}

export interface DesktopSkillCatalogDirent {
  readonly name: string;
  isDirectory(): boolean;
  isFile(): boolean;
}

export interface DesktopSkillCatalogPath {
...
export function createDesktopSkillFileSnapshotRuntime(
  options: DesktopSkillFileSnapshotRuntimeOptions,
): DesktopAgentSnapshotRuntime {
  return new DesktopSkillFileSnapshotRuntime({
...
export class DesktopSkillFileSnapshotRuntime implements DesktopAgentSnapshotRuntime {
  constructor(private readonly options: Required<DesktopSkillFileSnapshotRuntimeOptions>) {}
    ...
    return [
      {
        source: 'personal',
        kind: 'skills',
        dirPath: this.options.path.join(this.options.homeDir, '.neko', 'skills'),
      },
      {
        source: 'personal',
        kind: 'commands',
      ...
      },
      {
        source: 'project',
        kind: 'skills',
        dirPath: this.options.path.join(this.options.workspaceRoot, '.neko', 'skills'),
      },
      {
        source: 'project',
        kind: 'commands',
  ...
  ): Promise<DesktopAgentSkillsSnapshot> {
  ...
  private async readDirectory(dirPath: string): Promise<
    | { readonly ok: true; readonly entries: readonly DesktopSkillCatalogDirent[] }
  ...
  > {
    try {
      ...
      return {
        ...
        message: `Desktop Agent skill catalog scan failed for ${dirPath}: ${describeUnknownError(error)}`,
      };
    }
  }
  ...
  > {
    ...
    if (!description) {
      return {
        ok: false,
        message: `Desktop Agent skill catalog entry is missing description: ${filePath}`,
      };
    }
  ...
  > {
    ...
    if (!description) {
      return {
        ok: false,
        message: `Desktop Agent command catalog entry is missing description: ${filePath}`,
      };
    }
  ...
  private async readFrontmatter(filePath: string): Promise<
  ...
  > {
    try {
      ...
      if (!frontmatter) {
        return {
          ok: false,
          message: `Desktop Agent skill catalog entry is missing YAML frontmatter: ${filePath}`,
        };
      }
      ...
      return {
        ok: false,
        message: `Desktop Agent skill catalog entry failed to load ${filePath}: ${describeUnknownError(error)}`,
      };
    }
  }
}
