...
export interface DesktopProjectFileIoAdapterOptions {
  readonly workspaceRoot: string;
  readonly maxTextFileBytes?: number;
}

export interface DesktopProjectFileIoAdapter {
  readonly workspaceRoot: string;
...
export function createDesktopProjectFileIoAdapter(
  options: DesktopProjectFileIoAdapterOptions,
): DesktopProjectFileIoAdapter {
  const workspaceRoot = resolve(options.workspaceRoot);
  const maxTextFileBytes = options.maxTextFileBytes ?? DEFAULT_MAX_TEXT_FILE_BYTES;

  function resolveWorkspacePath(filePath: string): string {
    return resolveDesktopWorkspacePath(workspaceRoot, filePath);
  }
...
export function resolveDesktopWorkspacePath(workspaceRoot: string, filePath: string): string {
