		...
		<div className="space-y-1">
			...
			{result && (
				<div className="text-[10px] text-[var(--vscode-descriptionForeground)] px-1">
					LUFS: {result.integratedLufs.toFixed(1)} &rarr; {t('audio.gain')}: {result.recommendedGain > 0 ? '+' : ''}{result.recommendedGain.toFixed(1)} dB
				</div>
			)}

			{error && (
				<div className="text-[10px] text-[var(--vscode-errorForeground)] px-1">
					{error}
				</div>
			)}
		</div>
