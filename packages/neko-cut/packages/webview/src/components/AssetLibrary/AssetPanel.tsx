...
export const AssetPanel = memo(function AssetPanel({
  ...
  const { handleEntityClick, handleVariantClick, isEntitySelected, isVariantSelected } =
    useAssetSelection({
      entities,
      selection,
      setSelection,
  ...
  const handleVariantAttributeFilterChange = useCallback(
    (variantAttributes: VariantAttributeFilter) => {
      ...
      setFilter({
        ...filter,
        variantAttributes:
          Object.keys(cleanedAttributes).length > 0 ? cleanedAttributes : undefined,
      });
    },
    [filter, setFilter],
  );
  ...
  const handleVariantDragStart = useCallback(
    (e: React.DragEvent, entity: AssetEntity, variant: AssetVariant) => {
      // Check if the dragged variant is part of multi-selection
      const isVariantInSelection = selection.selectedItems.some(
        (item) =>
          item.type === 'variant' && item.entityId === entity.id && item.variantId === variant.id,
      );
      ...
      if (isVariantInSelection && selectedVariantItems.length > 0) {
        ...
        // Single item drag
        handleInternalDragStart(e, [
          { type: 'variant', entityId: entity.id, variantId: variant.id },
        ]);
      }
      ...
      if (isVariantInSelection && selectedVariantItems.length > 1) {
        ...
        dragEl.style.cssText =
          'position:absolute;top:-1000px;padding:4px 8px;background:#007acc;color:white;border-radius:4px;font-size:12px;';
  ...
  const handleMoveVariant = useCallback(
    (sourceEntityId: string, variantId: string) => {
      setEntitySelectorModal({
        isOpen: true,
  ...
  return (
    ...
    >
          ...
          <button
            ...
            title={
              comparison.isComparing
                ? t('assetLibrary.comparison.exit')
                : t('assetLibrary.comparison.enter')
            }
            aria-label={
              comparison.isComparing
                ? t('assetLibrary.comparison.exit')
      ...
      {/* Category Tabs */}
      <CategoryTabs activeCategory={filter.category} onCategoryChange={handleCategoryChange} />
