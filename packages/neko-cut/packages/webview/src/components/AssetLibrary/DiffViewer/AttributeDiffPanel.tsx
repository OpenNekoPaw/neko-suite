...
export const AttributeDiffPanel = memo(function AttributeDiffPanel({
  ...
  if (diffs.length === 0) {
    return <div className="p-4 text-center text-gray-400 text-sm">两个变体的属性完全相同</div>;
  }
  ...
  return (
    <div className="border border-gray-700 rounded-lg overflow-hidden">
      ...
      {/* Rows */}
      {diffs.map((diff) => (
        <div key={diff.attribute} className="grid grid-cols-3 text-sm border-t border-gray-700">
