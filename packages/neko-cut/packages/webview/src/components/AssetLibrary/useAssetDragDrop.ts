...
export function useAssetDragDrop({
  ...
  const handleInternalDragStart = useCallback(
    (e: React.DragEvent, sourceItems: SelectionItem[]) => {
  ...
  const isValidDropTarget = useCallback(
    (entityId: string, sourceItems: SelectionItem[]): boolean => {
      // Can't drop on self
   ...
   * Handle drag over entity
   */
  const handleDragOverEntity = useCallback((e: React.DragEvent, entityId: string) => {
   ...
   * Handle drop on entity
   */
  const handleDropOnEntity = useCallback(
