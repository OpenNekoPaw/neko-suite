...
export const EntityCard = memo(function EntityCard({
  ...
  const handleVariantContextMenu = useCallback(
    (e: React.MouseEvent, variantId: string) => {
      e.preventDefault();
      e.stopPropagation();
      setContextMenu({
        position: { x: e.clientX, y: e.clientY },
    ...
    <>
      <div
        ...
        style={{
          ...
          outline: isDropTarget ? '2px dashed var(--vscode-focusBorder)' : 'none',
          outlineOffset: '-2px',
        }}
        onClick={handleClick}
        ...
        {isExpanded && entity.variants.length > 0 && (
          ...
          >
            {entity.variants.map((variant) => {
              ...
              const showAsSelected = comparisonMode
                ? isSelectedForCompare
                : isVariantSelectedForMulti;
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    >
                      {variant.files.length > 1 &&
                        t('assetLibrary.fileCount', { count: variant.files.length })}
                      {variant.attributes.view &&
                        `${variant.files.length > 1 ? ' · ' : ''}${variant.attributes.view}`}
