...
通过聚焦的子 Skill 和可用 lifecycle capability 协调媒体转视频任务。它不是固定流水线：只选择当前最小必要的子 Skill，并让面向用户的结论基于 capability 证据。

## 工作流指引

1. 先判断输入来源：漫画、文档、图片、图片序列、已审阅的分镜 creative table、Canvas 分镜节点、动画计划或已生成媒体。
2. 激活生产类 Skill 前先判断意图：
   - 仅内容理解（描述、OCR、总结、提取文字、分格顺序、人物/场景分析、质量诊断、“阅读/分析前 N 页”）应停留在普通 content/perception 分析，不激活 media-to-video、comic-to-animation、comic-to-storyboard 或 storyboard-to-animation-plan。
...
3. 用运行时 skill catalog/context 查看可用相关 Skill，只在需要详细领域规则时激活聚焦子 Skill。
...
- Markdown 表格的 Canvas 审阅属于 Canvas authoring lifecycle capability。可用时通过运行时 adapter 传递原始 Markdown 和稳定 resource ref，但不要输出领域节点 JSON 或项目内部交接对象。有用的未知列应作为审阅 metadata 可见保留。
- 对 Cut、Sketch、Model 等项目文件的持久写入必须走 owning package 的 canonical authoring capability，并携带明确 target、reveal preference、稳定 source/ref 和 provenance。不要把 UI-bound import 命令、打开编辑器或显示预览当成持久交付。
...
- 不要检查 `.neko/.cache`、`.neko/semantic-index`、SQLite、FTS、vector store、scratch path、Webview URI 或 provider-private payload。聚焦 Skill 需要复用稳定来源范围的语义证据时，使用运行时语义证据复用能力。
- 批量生成、上色、破坏性替换时间线或长时间导出前必须请求用户确认，除非用户明确要求自动执行且策略允许。

## 相关 Skill 选择

- comic-to-storyboard：读取漫画页、分格/OCR 证据，输出包含生成有效提示词、资源 token，以及必要时仅作计划提示的 next action 的 Markdown creative table。
...
当存在稳定 source ref 和 range 时，在昂贵的长范围分析前先使用运行时语义证据复用能力。missing/stale 视觉证据再通过运行时 perception/content 能力获取。只有在用户确认后再使用生成 capability。只有校验通过且目标 capability 存在时，才使用 Canvas/Cut capability。
