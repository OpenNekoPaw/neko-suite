...
Convert one or more still images into reviewable shot planning tables. Use the runtime perception/content capability guidance to inspect images, describe visible evidence, and produce a Markdown creative table when the user wants storyboard or shot planning.
...
- Use the same prompt-slot-aware storyboard headers as comic-to-storyboard when the output is a storyboard table. Prefer the primary fields `scene`, `shot`, `source`, `imagePrompt`, `videoPrompt`, `duration`, and `dialogue`.
- For chat output, include `scene` + `shot` and either `source` or at least one prompt slot; open review metadata columns are allowed when useful.
...
- Add extension columns such as `sourcePanel`, `decision`, `decisionReason`, `referenceImage`, `styleRef`, `requiresSplit`, `requiresInpaint`, `requiresOutpaint`, `contentType`, `nextAction`, or `risk` only when they help review or planning.
- Do not add a status column by default. Status belongs to Canvas review state, Agent async tasks, or execution summaries, not the primary shot planning table.
- Do not claim generated images or videos exist until a generation capability returns them.
