...
function readSupportedElementUpdates(
  ...
  if (muted !== undefined) next.muted = muted;
  ...
  if (startTime !== undefined) next.startTime = startTime;
  ...
  if (trimStart !== undefined) next.trimStart = trimStart;

  const trimEnd = updates ? readNumber(updates, 'trimEnd') : readNumber(parameters, 'trimEnd');
  if (trimEnd !== undefined) next.trimEnd = trimEnd;
