...
export function AnimationPanel({ onPlay, onStop, onSeek }: AnimationPanelProps) {
  ...
  if (!puppetLoaded) return null;

  return (
    <div className="sketch-panel" role="region" aria-label={t('puppet.panel.animation')}>
      {/* Header */}
      <div className="flex items-center justify-between mb-1">
        <h3 className="sketch-panel-title m-0">{t('puppet.panel.animation')}</h3>
        <div className="flex items-center gap-1">
          <PhysicsToggle />
          {streamConnected && (
            <span
              title={t('puppet.animation.liveStream')}
              className="w-2 h-2 rounded-full bg-green-500 inline-block"
              aria-label={t('puppet.animation.liveStream')}
            />
          )}
        </div>
      </div>
      ...
      {/* Clip list */}
      {animations.length === 0 ? (
        <p className="text-xs opacity-50 px-1">{t('puppet.animation.noClips')}</p>
      ) : (
        ...
        <div className="flex flex-col gap-1 px-1">
          <div className="flex items-center gap-1">
            <button
              ...
              aria-label={isPlaying ? t('puppet.animation.stop') : t('puppet.animation.play')}
            >
              {isPlaying ? '■' : '▶'}
            </button>
...
function PhysicsToggle() {
  const { t } = useTranslation();
  const isPlaying = usePuppetStore((s) => s.isPlayingPhysics);
  const setPlaying = usePuppetStore((s) => s.setPlayingPhysics);
  ...
  return (
    <button
      ...
      title={isPlaying ? t('puppet.animation.disablePhysics') : t('puppet.animation.enablePhysics')}
      aria-label={
        isPlaying ? t('puppet.animation.disablePhysicsSim') : t('puppet.animation.enablePhysicsSim')
      }
