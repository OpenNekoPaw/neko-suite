import type { MessageBundle } from '@neko/shared';

export const en: MessageBundle = {
  // Toolbar tools
  'sketch.toolbar.brush': 'Brush',
  'sketch.toolbar.eraser': 'Eraser',
  'sketch.toolbar.select': 'Select',
  'sketch.toolbar.lasso': 'Lasso',
  'sketch.toolbar.wand': 'Magic Wand',
  'sketch.toolbar.move': 'Move',
  'sketch.toolbar.shape': 'Shape',
  'sketch.toolbar.vector': 'Vector Nodes',
  'sketch.toolbar.transform': 'Transform',
  'sketch.toolbar.eyedropper': 'Eyedropper',
  'sketch.toolbar.fill': 'Fill',
  'sketch.toolbar.gradient': 'Gradient',
  'sketch.toolbar.text': 'Text',
  'sketch.toolbar.clone': 'Clone',
  'sketch.toolbar.zoom': 'Zoom',
  'sketch.toolbar.export': 'Export image',
  'sketch.toolbar.package': 'Package project',
  'sketch.toolbar.ariaLabel': 'Drawing tools',
  'sketch.hostAdapter.settings': 'Settings',
  'sketch.hostAdapter.dock': '{label} adapter',
  'sketch.hostAdapter.package': 'Package',
  'sketch.hostAdapter.panel': 'Panel',
  'sketch.hostAdapter.runtime': 'Runtime',
  'sketch.hostAdapter.file': 'File',
  'sketch.sidebar.toggle': 'Toggle Sidebar',
  'sketch.common.enabled': 'Enabled',
  'sketch.rightDock.mode.label': 'Creation mode',
  'sketch.rightDock.mode.basic': 'Basic',
  'sketch.rightDock.mode.basic.description': 'AI-assisted drawing and core painting panels',
  'sketch.rightDock.mode.professional': 'Professional',
  'sketch.rightDock.mode.professional.description':
    'Animation, filters, scene, and precision tools',

  // Layer panel
  'sketch.layer.new': 'New Layer',
  'sketch.layer.vectorDefaultName': 'Vector Layer {index}',
  'sketch.layer.delete': 'Delete Layer',
  'sketch.layer.duplicate': 'Duplicate Layer',
  'sketch.layer.mergeDown': 'Merge Down',
  'sketch.layer.flatten': 'Flatten Visible',
  'sketch.layer.group': 'Group Layers',
  'sketch.layer.ungroup': 'Ungroup',
  'sketch.panel.layers': 'Layers',
  'sketch.panel.advanced': 'Advanced',
  'sketch.layer.add': 'Add layer',
  'sketch.layer.addVector': 'Add vector layer',
  'sketch.layer.addBackground': 'Add background',
  'sketch.layer.show': 'Show layer',
  'sketch.layer.hide': 'Hide layer',
  'sketch.layer.lock': 'Lock layer',
  'sketch.layer.unlock': 'Unlock layer',
  'sketch.layer.remove': 'Remove layer',
  'sketch.layer.moveUp': 'Move Up',
  'sketch.layer.moveDown': 'Move Down',
  'sketch.layer.addAdjustment': 'Add adjustment layer',
  'sketch.layer.adjustment.brightnessContrast': 'Brightness / Contrast',
  'sketch.layer.adjustment.hueSaturation': 'Hue / Saturation',
  'sketch.layer.adjustment.exposure': 'Exposure',
  'sketch.layer.adjustment.temperature': 'Temperature',
  'sketch.layer.createClippingMask': 'Create Clipping Mask',
  'sketch.layer.releaseClippingMask': 'Release Clipping Mask',
  'sketch.layer.lockAlpha': 'Lock Alpha',
  'sketch.layer.unlockAlpha': 'Unlock Alpha',
  'sketch.layer.badge.adjustment': 'ADJ',
  'sketch.layer.badge.adjustmentTitle': 'Adjustment layer',
  'sketch.layer.badge.clippingMask': 'Clip',
  'sketch.layer.badge.clippingMaskTitle': 'Clipping mask',
  'sketch.layer.badge.alphaLock': 'Alpha',
  'sketch.layer.badge.alphaLockTitle': 'Alpha lock',

  // Brush panel
  'sketch.brush.pencil': 'Pencil',
  'sketch.brush.pen': 'Pen',
  'sketch.brush.watercolor': 'Watercolor',
  'sketch.brush.airbrush': 'Airbrush',
  'sketch.brush.eraser': 'Eraser',
  'sketch.brush.marker': 'Marker',
  'sketch.brush.pixel': 'Pixel',
  'sketch.brush.stamp': 'Texture Stamp',
  'sketch.panel.brush': 'Brush Settings',
  'sketch.brush.type': 'Type',
  'sketch.brush.size': 'Size',
  'sketch.brush.opacity': 'Opacity',
  'sketch.brush.hardness': 'Hardness',
  'sketch.brush.spacing': 'Spacing',
  'sketch.brush.symmetry': 'Symmetry',
  'sketch.brush.symmetry.off': 'Off',
  'sketch.brush.symmetry.vertical': 'Vertical',
  'sketch.brush.symmetry.horizontal': 'Horizontal',
  'sketch.brush.symmetry.both': 'Both',
  'sketch.brush.symmetry.radial': 'Radial',
  'sketch.brush.stampPattern': 'Stamp texture',
  'sketch.brush.stampPattern.builtIn': 'Built-in',
  'sketch.brush.stampPattern.assets': 'Assets',
  'sketch.brush.stampPattern.grain': 'Grain',
  'sketch.brush.stampPattern.crosshatch': 'Crosshatch',
  'sketch.brush.stampPattern.bristle': 'Bristle',
  'sketch.brush.stampAssets': 'Assets',
  'sketch.brush.stampAsset.import': 'Import',
  'sketch.brush.stampAsset.remove': 'Remove',

  // Fill panel
  'sketch.panel.fill': 'Fill',
  'sketch.fill.pattern': 'Pattern',
  'sketch.fill.pattern.solid': 'Solid',
  'sketch.fill.pattern.checker': 'Checker',
  'sketch.fill.pattern.dots': 'Dots',
  'sketch.fill.pattern.diagonal': 'Diagonal',
  'sketch.fill.patternSize': 'Size: {size}px',

  // Color panel
  'sketch.panel.color': 'Color',
  'sketch.color.brushColor': 'Brush color',
  'sketch.color.colorLabel': 'Color {color}',

  // Filter panel
  'sketch.panel.filters': 'Filters',
  'sketch.filter.addFilter': 'Add Filter...',
  'sketch.filter.noFilters': 'No filters applied',
  'sketch.filter.remove': 'Remove filter',
  'sketch.filter.category.blur': 'Blur',
  'sketch.filter.category.color': 'Color',
  'sketch.filter.category.distort': 'Distort',
  'sketch.filter.category.stylize': 'Stylize',
  'sketch.filter.preset.warmSunset': 'Warm Sunset',
  'sketch.filter.preset.coolBlue': 'Cool Blue',
  'sketch.filter.preset.vintageFilm': 'Vintage Film',
  'sketch.filter.preset.highContrastBw': 'High Contrast B&W',

  // Particle panel
  'sketch.panel.particles': 'Particles',
  'sketch.particle.togglePreview': 'Toggle particle preview',
  'sketch.particle.addEmitter': 'Add emitter',
  'sketch.particle.noEmitters': 'No emitters',
  'sketch.particle.remove': 'Remove {name}',
  'sketch.particle.shape': 'Shape',
  'sketch.particle.shapeLabel': 'Emitter shape',
  'sketch.particle.shape.point': 'Point',
  'sketch.particle.shape.line': 'Line',
  'sketch.particle.shape.circle': 'Circle',
  'sketch.particle.shape.rect': 'Rect',
  'sketch.particle.blend': 'Blend',
  'sketch.particle.blendLabel': 'Blend mode',
  'sketch.particle.blend.additive': 'Additive',
  'sketch.particle.blend.normal': 'Normal',
  'sketch.particle.blend.multiply': 'Multiply',
  'sketch.particle.rate': 'Rate',
  'sketch.particle.speed': 'Speed',
  'sketch.particle.size': 'Size',
  'sketch.particle.direction': 'Dir',
  'sketch.particle.spread': 'Spread',

  // Atmosphere panel
  'sketch.panel.atmosphere': 'Atmosphere',
  'sketch.atmosphere.preset': 'Preset',
  'sketch.atmosphere.presetLabel': 'Atmosphere preset',
  'sketch.atmosphere.preset.none': 'None',
  'sketch.atmosphere.preset.fog': 'Fog',
  'sketch.atmosphere.preset.rain': 'Rain',
  'sketch.atmosphere.preset.snow': 'Snow',
  'sketch.atmosphere.preset.fireflies': 'Fireflies',
  'sketch.atmosphere.preset.dust': 'Dust',
  'sketch.atmosphere.intensity': 'Intensity',
  'sketch.atmosphere.windX': 'Wind X',

  // Scene panel
  'sketch.panel.scene': 'Scene',
  'sketch.scene.selectScene': 'Select scene...',
  'sketch.scene.newScene': 'New scene',
  'sketch.scene.createScene': 'Create scene',
  'sketch.scene.useTemplate': 'Or use a template:',
  'sketch.scene.zoom': 'Zoom',
  'sketch.scene.cameraZoom': 'Camera zoom',
  'sketch.scene.layers': 'Layers',
  'sketch.scene.addLayer': 'Add scene layer',
  'sketch.scene.parallaxX': 'Parallax X',
  'sketch.scene.removeLayer': 'Remove {name}',
  'sketch.scene.deleteScene': 'Delete scene',
  'sketch.scene.activeScene': 'Active scene',
  'sketch.scene.canvasLayer': 'Canvas layer',
  'sketch.scene.canvasLayerNone': '(none)',
  'sketch.scene.defaultName': 'Scene {index}',
  'sketch.scene.defaultLayerName': 'Layer {index}',
  'sketch.scene.lightsLayer': 'Lights',
  'sketch.scene.lighting': 'Lighting',
  'sketch.scene.ambient': 'Ambient',
  'sketch.scene.ambientIntensity': 'Ambient intensity',
  'sketch.scene.ambientColor': 'Ambient color',
  'sketch.scene.lights': 'Lights',
  'sketch.scene.addPointLight': 'Add point light',
  'sketch.scene.addDirectionalLight': 'Add directional light',
  'sketch.scene.addSpotLight': 'Add spot light',
  'sketch.scene.removeLight': 'Remove light',

  // Light inspector
  'sketch.light.selected': '{type} Light',
  'sketch.light.type': 'Type',
  'sketch.light.type.point': 'Point',
  'sketch.light.type.directional': 'Directional',
  'sketch.light.type.spot': 'Spot',
  'sketch.light.type.pointShort': 'P',
  'sketch.light.type.directionalShort': 'D',
  'sketch.light.type.spotShort': 'S',
  'sketch.light.xPosition': 'Light X position',
  'sketch.light.yPosition': 'Light Y position',
  'sketch.light.color': 'Color',
  'sketch.light.intensity': 'Intensity',
  'sketch.light.radius': 'Radius',
  'sketch.light.direction': 'Direction',
  'sketch.light.cone': 'Cone',
  'sketch.light.softness': 'Softness',
  'sketch.light.height': 'Height',

  // Frame controls
  'sketch.panel.frames': 'Frames',
  'sketch.frame.newFrameLayer': '+ New Frame Layer',
  'sketch.frame.frameLayer': 'Frame layer',
  'sketch.frame.addFrameLayer': 'Add frame layer',
  'sketch.frame.addFrame': 'Add frame',
  'sketch.frame.addFrameKey': 'Add frame (F6)',
  'sketch.frame.addFrameButton': '+ Frame',
  'sketch.frame.addBlank': 'Add blank frame',
  'sketch.frame.addBlankButton': '+ Blank',
  'sketch.frame.duplicate': 'Duplicate frame',
  'sketch.frame.duplicateTooltip': 'Duplicate current frame',
  'sketch.frame.delete': 'Delete frame',
  'sketch.frame.deleteTooltip': 'Delete current frame',
  'sketch.frame.exportSpriteSheet': 'Export Sprite Sheet',
  'sketch.frame.exportSpriteSheetTooltip': 'Export as sprite sheet',

  // Vector toolbar
  'sketch.panel.vector': 'Vector Tools',
  'sketch.vector.sides': 'Sides',
  'sketch.vector.sidesLabel': 'Polygon sides',
  'sketch.vector.points': 'Points',
  'sketch.vector.pointsLabel': 'Star points',
  'sketch.vector.handleMode': 'Handle Mode',
  'sketch.vector.handleMode.corner': 'Corner',
  'sketch.vector.handleMode.smooth': 'Smooth',
  'sketch.vector.handleMode.mirrored': 'Mirrored',
  'sketch.vector.closePath': 'Close Path',
  'sketch.vector.openPath': 'Open Path',
  'sketch.vector.reversePath': 'Reverse Path',
  'sketch.vector.duplicatePath': 'Duplicate Path',

  // Text overlay
  'sketch.text.content': 'Text',
  'sketch.text.size': 'Size',
  'sketch.text.color': 'Color',
  'sketch.text.bold': 'Bold',
  'sketch.text.italic': 'Italic',
  'sketch.text.apply': 'Apply',
  'sketch.text.cancel': 'Cancel',

  // Palette panel
  'sketch.panel.palette': 'Palette',
  'sketch.palette.select': 'Select palette',
  'sketch.palette.new': 'New',
  'sketch.palette.addColor': '+ Color',
  'sketch.palette.import': 'Import',
  'sketch.palette.export': 'Export',
  'sketch.palette.customName': 'Custom Palette',
  'sketch.palette.importFailed': 'Could not import palette',

  // Perspective grid panel
  'sketch.panel.perspective': 'Perspective Grid',
  'sketch.perspective.mode': 'Mode',
  'sketch.perspective.mode.one': '1 Point',
  'sketch.perspective.mode.two': '2 Point',
  'sketch.perspective.mode.three': '3 Point',
  'sketch.perspective.divisions': 'Divisions',
  'sketch.perspective.opacity': 'Opacity',
  'sketch.perspective.snap': 'Snap shapes',
  'sketch.perspective.reset': 'Reset Grid',

  // AI panel
  'sketch.panel.ai': 'AI',
  'sketch.ai.operation': 'Operation',
  'sketch.ai.prompt': 'Prompt',
  'sketch.ai.promptPlaceholder': 'Describe the edit...',
  'sketch.ai.openAgent': 'Open in Agent',
  'sketch.ai.operation.generate': 'Generate Layer',
  'sketch.ai.operation.smartSelection': 'Smart Selection',
  'sketch.ai.operation.inpaint': 'Inpaint',
  'sketch.ai.operation.styleTransfer': 'Style Transfer',
  'sketch.ai.operation.upscale': 'Upscale',
  'sketch.ai.operation.lineartColorize': 'Line Art Colorize',
  'sketch.ai.operation.autoLayer': 'Auto Layer',
  'sketch.ai.scope': 'Scope',
  'sketch.ai.scope.canvas': 'Canvas',
  'sketch.ai.scope.layer': 'Active Layer',
  'sketch.ai.negativePrompt': 'Negative Prompt',
  'sketch.ai.strength': 'Strength',
  'sketch.ai.style': 'Style',
  'sketch.ai.scale': 'Scale',
  'sketch.ai.layerName': 'Layer Name',
  'sketch.ai.palette': 'Palette',
  'sketch.ai.palettePlaceholder': '#ffcc66, teal, ink black',
  'sketch.ai.autoLayerTargets': 'Layer Targets',
  'sketch.ai.autoLayer.lineart': 'Line Art',
  'sketch.ai.autoLayer.flatcolor': 'Flat Color',
  'sketch.ai.autoLayer.shadow': 'Shadow',
  'sketch.ai.autoLayer.highlight': 'Highlight',
  'sketch.ai.style.anime': 'Anime',
  'sketch.ai.style.oilPainting': 'Oil Painting',
  'sketch.ai.style.watercolor': 'Watercolor',
  'sketch.ai.style.pixelArt': 'Pixel Art',
  'sketch.ai.style.sketch': 'Sketch',
  'sketch.ai.style.comic': 'Comic',
  'sketch.ai.style.ghibli': 'Ghibli',

  // File
  'sketch.file.import': 'Import',
  'sketch.file.export': 'Export',

  // Frame timeline
  'sketch.timeline.stop': 'Stop playback',
  'sketch.timeline.play': 'Play animation',
  'sketch.timeline.fps': 'FPS:',
  'sketch.timeline.fpsLabel': 'Frames per second',
  'sketch.timeline.onionSkin': 'Toggle onion skin (O)',
  'sketch.timeline.onionSkinLabel': 'Toggle onion skin',
  'sketch.timeline.frames': 'Animation frames',
  'sketch.timeline.showFrameTimeline': 'Show frame timeline',
  'sketch.timeline.hideFrameTimeline': 'Hide frame timeline',
  'sketch.timeline.frame': 'Frame {index}',
  'sketch.timeline.frameKey': 'Frame {index} (key)',

  // Scene templates
  'sketch.template.platformer': 'Platformer (3-layer parallax)',
  'sketch.template.topdownRpg': 'Top-Down RPG (2-layer)',
  'sketch.template.visualNovel': 'Visual Novel (BG + FG)',
  'sketch.template.sideScroller': 'Side Scroller (4-layer)',
  'sketch.template.layer.sky': 'Sky',
  'sketch.template.layer.mountains': 'Mountains',
  'sketch.template.layer.foreground': 'Foreground',
  'sketch.template.layer.ground': 'Ground',
  'sketch.template.layer.objects': 'Objects',
  'sketch.template.layer.background': 'Background',
  'sketch.template.layer.characters': 'Characters',
  'sketch.template.layer.farBg': 'Far BG',
  'sketch.template.layer.nearBg': 'Near BG',
  'sketch.template.layer.gameplay': 'Gameplay',

  // Default names
  'sketch.layer.defaultName': 'Layer {index}',
  'sketch.layer.copySuffix': '{name} copy',
  'sketch.layer.defaultGroup': 'Group',

  // Filter names
  'sketch.filter.gaussianBlur': 'Gaussian Blur',
  'sketch.filter.brightnessContrast': 'Brightness / Contrast',
  'sketch.filter.hueSaturation': 'Hue / Saturation',
  'sketch.filter.sharpen': 'Sharpen',
  'sketch.filter.vignette': 'Vignette',
  'sketch.filter.chromaticAberration': 'Chromatic Aberration',
  'sketch.filter.exposure': 'Exposure',
  'sketch.filter.colorTemperature': 'Color Temperature',
  'sketch.filter.gradientMap': 'Gradient Map',
  'sketch.filter.glow': 'Glow',
  'sketch.filter.filmGrain': 'Film Grain',
  'sketch.filter.halftone': 'Halftone',
  'sketch.filter.ssao': 'Screen-Space AO',

  // Filter parameter labels
  'sketch.filter.param.radius': 'Radius',
  'sketch.filter.param.brightness': 'Brightness',
  'sketch.filter.param.contrast': 'Contrast',
  'sketch.filter.param.hue': 'Hue',
  'sketch.filter.param.saturation': 'Saturation',
  'sketch.filter.param.lightness': 'Lightness',
  'sketch.filter.param.amount': 'Amount',
  'sketch.filter.param.softness': 'Softness',
  'sketch.filter.param.offset': 'Offset',
  'sketch.filter.param.stops': 'Stops',
  'sketch.filter.param.temperature': 'Temperature',
  'sketch.filter.param.intensity': 'Intensity',
  'sketch.filter.param.cellSize': 'Cell Size',
  'sketch.filter.param.angle': 'Angle',
  'sketch.filter.param.shadowColor': 'Shadows',
  'sketch.filter.param.highlightColor': 'Highlights',
  'sketch.filter.param.bias': 'Bias',

  // Filter toggle
  'sketch.filter.toggle': 'Toggle {name}',

  // Sprite sheet player
  'sketch.panel.spritesheet': 'Sprite Sheet Player',
  'sketch.spritesheet.cols': 'Cols',
  'sketch.spritesheet.colsLabel': 'Grid columns',
  'sketch.spritesheet.rows': 'Rows',
  'sketch.spritesheet.rowsLabel': 'Grid rows',
  'sketch.spritesheet.importLabel': 'Import sprite sheet',
  'sketch.spritesheet.dropHint': 'Drop PNG (+ JSON) or click to import',
  'sketch.spritesheet.reimport': 'Drop or click to replace',
  'sketch.spritesheet.noFrames': 'No frames found in sprite sheet',
  'sketch.spritesheet.preview': 'Frame preview',
  'sketch.spritesheet.seek': 'Seek frame',
  'sketch.spritesheet.fps': 'FPS',
  'sketch.spritesheet.fpsLabel': 'Playback FPS',
  'sketch.spritesheet.play': 'Play animation',
  'sketch.spritesheet.stop': 'Stop animation',
  'sketch.spritesheet.frameList': 'Frame thumbnails',

  // Status bar
  'sketch.status.ready': 'Ready',
  'sketch.status.layers': 'layers',

  // Canvas context menu
  'sketch.canvas.undo': 'Undo',
  'sketch.canvas.redo': 'Redo',
  'sketch.canvas.selectAll': 'Select All',
  'sketch.canvas.deselect': 'Deselect',
  'sketch.canvas.zoomIn': 'Zoom In',
  'sketch.canvas.zoomOut': 'Zoom Out',
  'sketch.canvas.resetZoom': 'Reset Zoom',
  'sketch.canvas.rotateViewLeft': 'Rotate View Left',
  'sketch.canvas.rotateViewRight': 'Rotate View Right',
  'sketch.canvas.resetRotation': 'Reset Rotation',
  'sketch.canvas.flipH': 'Flip Horizontal',
  'sketch.canvas.flipV': 'Flip Vertical',
  'sketch.canvas.rotate90': 'Rotate 90° CW',
  'sketch.canvas.clearLayer': 'Clear Layer',

  // Tool labels
  'sketch.tool.eraser': 'Eraser',
};
