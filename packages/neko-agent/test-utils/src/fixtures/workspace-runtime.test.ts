...
describe('agent workspace runtime fixtures', () => {
  it('writes standard user/workspace config, skills, commands, task, and cache files', async () => {
    ...
    await expect(fs.stat(fixture.paths.userConfigPath)).resolves.toMatchObject({ isFile: expect.any(Function) });
    await expect(fs.stat(fixture.paths.workspaceConfigPath)).resolves.toMatchObject({ isFile: expect.any(Function) });
    await expect(
      fs.readFile(path.join(fixture.paths.workspaceSkillsDir, 'project-review', 'SKILL.md'), 'utf-8'),
    ...
    await expect(fs.readFile(fixture.paths.taskRecordsPath, 'utf-8')).resolves.toContain(
      'workspace-task-1',
    );
