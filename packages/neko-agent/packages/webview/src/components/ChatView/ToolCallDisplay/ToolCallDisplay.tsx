...
function ToolCallDisplayComponent({ toolCall }: ToolCallDisplayProps) {
  ...
  const handleConfirm = useCallback((approved: boolean) => {
    logger.info('handleConfirm called:', { toolCallId: toolCall.id, toolName: toolCall.name, approved });
  ...
  const imageUrls = (toolCall.result?.success && shouldShowMediaPreview) ? extractImageUrls(toolCall.result.data) : [];

  const isVideoTool = VIDEO_GENERATION_TOOLS.includes(toolCall.name);
  const videoUrls = (toolCall.result?.success && shouldShowMediaPreview) ? extractVideoUrls(toolCall.result.data) : [];

  const isAudioTool = AUDIO_GENERATION_TOOLS.includes(toolCall.name);
  const audioUrls = (toolCall.result?.success && shouldShowMediaPreview) ? extractAudioUrls(toolCall.result.data) : [];
  ...
  if (needsConfirmation) {
    logger.info('Rendering confirmation UI for:', { toolCallId: toolCall.id, toolName: toolCall.name });
    return (
      <div className="my-2 border border-[var(--vscode-inputValidation-warningBorder)] rounded-md overflow-hidden">
        <div className="flex items-center gap-2 px-3 py-2 bg-[var(--vscode-inputValidation-warningBackground)]">
          ...
          <span className="font-medium text-[12px] text-[var(--vscode-foreground)]">Tool Confirmation Required</span>
        </div>
        <div className="px-3 py-2 bg-[var(--vscode-editor-background)]">
          <div className="flex items-center gap-2 mb-2">
            ...
            {toolCall.confirmation?.action && (
              <span className="text-[11px] text-[var(--vscode-descriptionForeground)]">{toolCall.confirmation.action}</span>
            )}
          </div>
          {toolCall.confirmation?.description && (
            <p className="text-[11px] text-[var(--vscode-foreground)] mb-2">{toolCall.confirmation.description}</p>
          )}
          {summary && (
            <div className="text-[10px] font-mono text-[var(--vscode-descriptionForeground)] mb-2 truncate">{summary}</div>
          )}
          {hasExpandableContent && (
            <div className="mb-2">
              <button onClick={toggleExpand} className="flex items-center gap-1 text-[10px] text-[var(--vscode-textLink-foreground)] hover:underline">
          ...
          <div className="flex items-center gap-2 pt-2 border-t border-[var(--vscode-panel-border)]">
            <button onClick={() => handleConfirm(true)} className="px-3 py-1 text-[11px] font-medium rounded bg-[var(--vscode-button-background)] text-[var(--vscode-button-foreground)] hover:bg-[var(--vscode-button-hoverBackground)] transition-colors">Allow</button>
            <button onClick={() => handleConfirm(false)} className="px-3 py-1 text-[11px] font-medium rounded bg-[var(--vscode-button-secondaryBackground)] text-[var(--vscode-button-secondaryForeground)] hover:bg-[var(--vscode-button-secondaryHoverBackground)] transition-colors">Deny</button>
            <span className="flex-1" />
            <span className="text-[10px] text-[var(--vscode-descriptionForeground)]">Press Enter to allow, Esc to deny</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="my-1">
      ...
      >
        {isPending && <ToolLoadingSpinner className="w-3 h-3 text-[var(--vscode-textLink-foreground)] shrink-0" />}
        ...
        <span className="font-medium text-[var(--vscode-foreground)] shrink-0">{toolCall.name}</span>

        {summary && <span className="text-[var(--vscode-descriptionForeground)] font-mono truncate">{summary}</span>}
        <span className="flex-1" />

        {isFileTool && filePath && isSuccess && (
          <button
            onClick={(e) => { e.stopPropagation(); handleOpenFile(filePath); }}
            ...
            title={`Open ${filePath}`}
          >
            <FileIcon className="w-3 h-3" /><span>Open</span>
          </button>
        )}

        {toolCall.result?.duration && (
          <span className="text-[var(--vscode-descriptionForeground)] text-[10px] shrink-0">{toolCall.result.duration}ms</span>
        )}
      ...
      {isExpanded && (
        <div className="mt-1 ml-4 pl-2 border-l border-[var(--vscode-panel-border)] text-[10px]">
          {Object.keys(toolCall.arguments).length > 0 && (
            <div className="mb-2">
              <div className="text-[var(--vscode-descriptionForeground)] opacity-70 mb-0.5">{t('chat.toolCall.args')}</div>
              <pre className="p-1.5 bg-[var(--vscode-editor-background)] rounded overflow-x-auto font-mono border border-[var(--vscode-panel-border)] max-h-[150px] w-full max-w-full">{argsJson}</pre>
            </div>
          )}
          {resultJson && (
            <div>
              <div className="text-[var(--vscode-descriptionForeground)] opacity-70 mb-0.5">Result</div>
              <pre className="p-1.5 bg-[var(--vscode-editor-background)] rounded overflow-x-auto font-mono border border-[var(--vscode-panel-border)] max-h-[150px] w-full max-w-full">{resultJson}</pre>
            </div>
          )}
        </div>
      )}
      ...
      {isImageTool && imageUrls.length > 0 && (
        <div className="mt-2 space-y-2">
          {imageUrls.map((url, index) => (
            <ImagePreview key={index} src={url} alt={`Generated image ${index + 1}`} name={`generated_${index + 1}.png`} localPath={localPaths[index] || localPaths[0]} />
          ))}
        </div>
      )}
      {isVideoTool && videoUrls.length > 0 && (
        <div className="mt-2 space-y-2">
          {videoUrls.map((url, index) => (
            <VideoPlayer key={index} src={url} title={`generated_${index + 1}.mp4`} localPath={localPaths[index] || localPaths[0]} />
          ))}
        </div>
      )}
      {isAudioTool && audioUrls.length > 0 && (
        <div className="mt-2 space-y-2">
          {audioUrls.map((url, index) => (
            <AudioPlayer key={index} src={url} title={`generated_${index + 1}.mp3`} localPath={localPaths[index] || localPaths[0]} />
          ))}
        </div>
      )}
    </div>
