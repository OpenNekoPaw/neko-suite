...
const KEYWORDS: Record<string, Set<string>> = {
  typescript: new Set([
    ...
    'implements', 'function', 'const', 'let', 'var', 'return', 'if', 'else',
    'for', 'while', 'do', 'switch', 'case', 'break', 'continue', 'throw',
    'try', 'catch', 'finally', 'new', 'this', 'super', 'typeof', 'instanceof',
    'async', 'await', 'yield', 'void', 'null', 'undefined', 'true', 'false',
    'as', 'in', 'of', 'readonly', 'private', 'protected', 'public', 'static',
    'abstract', 'override', 'declare', 'enum', 'namespace', 'module',
  ...
  javascript: new Set([
    ...
    'instanceof', 'async', 'await', 'yield', 'void', 'null', 'undefined',
    'true', 'false', 'class', 'extends', 'delete', 'in', 'of', 'default',
  ]),
  python: new Set([
    'import', 'from', 'def', 'class', 'return', 'if', 'elif', 'else',
    'for', 'while', 'break', 'continue', 'try', 'except', 'finally',
    'with', 'as', 'raise', 'pass', 'yield', 'lambda', 'and', 'or', 'not',
  ...
  rust: new Set([
    ...
    'continue', 'async', 'await', 'move', 'ref', 'where', 'unsafe',
    'true', 'false', 'as', 'in', 'dyn', 'extern',
  ]),
  bash: new Set([
    'if', 'then', 'else', 'elif', 'fi', 'for', 'while', 'do', 'done',
    'case', 'esac', 'function', 'return', 'exit', 'export', 'local',
    'readonly', 'declare', 'unset', 'source', 'echo', 'cd', 'ls',
    'cat', 'grep', 'sed', 'awk', 'find', 'rm', 'cp', 'mv', 'mkdir',
    'true', 'false',
  ]),
};
...
export function highlightLine(line: string, language?: string): HighlightToken[] {
  ...
  const tokenPattern = /("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|`(?:[^`\\]|\\.)*`|\/\/.*$|\/\*[\s\S]*?\*\/|#.*$|\d+(?:\.\d+)?|\b\w+\b|[^\s\w]|\s+)/g;
