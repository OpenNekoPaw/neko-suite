...
import type { RoutedPipelineResult } from './orchestrator-bootstrap';
...
export interface PipelineLifecycleBridgeDeps {
  planStoreWriter: PlanStoreWriter;
  getWebview: () => vscode.Webview | undefined;
}

export class PipelineLifecycleBridge {
  constructor(private readonly deps: PipelineLifecycleBridgeDeps) {}
   ...
   * dispatchApprovedPlan and the external `startRoutedPipeline` entry
   * point in index.ts).  Safe to call multiple times per pipeline —
   * progress-bridge + registerActivePipeline are idempotent.
   */
