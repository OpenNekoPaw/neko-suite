...
export class DocumentResourceCacheProvider implements ResourceCacheProvider {
  ...
  private readonly reader: IDocumentReaderService;
...
function createDocumentResourceRelativePath(
  ...
  const safeEntry = sanitizePathPart(
    entryPath ? path.basename(entryPath, path.extname(entryPath)) : ref.id,
  );
  return path.join('documents', ref.id, `${safeEntry}${ext}`);
}
