...
export class ProjectIndexCoordinator implements vscode.Disposable {
  ...
  private readonly logger = getLogger('ProjectIndexCoordinator');
  private readonly onDidChangeEmitter = new vscode.EventEmitter<ProjectIndexChangeEvent>();
  ...
  async ensureInitialized(projectRoot?: string): Promise<void> {
    const roots = projectRoot ? [projectRoot] : getWorkspaceRoots();
  ...
  async resolveContext(query: ProjectSearchQuery): Promise<ProjectSearchQueryContext> {
    return resolveProjectSearchContext(query);
  }
  ...
  async query(query: ProjectSearchQuery): Promise<{
    ...
    settled.forEach((result, index) => {
      ...
      this.logger.warn(`Project search partition failed: ${adapter?.partition ?? 'unknown'}`, {
        error: result.reason,
      });
    });
  ...
  getStatus(projectRoot?: string): readonly ProjectSearchPartitionStatusSnapshot[] {
    const root = projectRoot ?? getWorkspaceRoots()[0] ?? '';
