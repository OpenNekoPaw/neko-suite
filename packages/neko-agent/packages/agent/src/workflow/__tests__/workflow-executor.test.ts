...
import type { IWorkflowStage, IParallelStage, WorkflowConfig, WorkflowContext } from '../types';

// Helper: create a linear stage
function linearStage(
  name: string,
  gate: 'auto' | 'confirm' = 'auto',
  executeFn?: (ctx: WorkflowContext) => Promise<WorkflowContext>,
): IWorkflowStage {
  return {
    name,
    type: 'linear',
...
function defaultConfig(overrides?: Partial<WorkflowConfig>): WorkflowConfig {
...
describe('WorkflowExecutor', () => {
  ...
  it('should throw for reactive stages (not implemented)', async () => {
    const executor = new WorkflowExecutor();
    const reactiveStage: IWorkflowStage = {
      name: 'reactive',
      type: 'reactive',
      gate: 'auto',
      execute: async (ctx) => ctx,
