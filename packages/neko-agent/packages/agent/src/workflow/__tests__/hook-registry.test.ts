/**
 * WorkflowHookRegistry unit tests
 */

import { describe, it, expect, vi } from 'vitest';
import { WorkflowHookRegistry } from '../hook-registry';
import type { WorkflowContext } from '../types';

function createCtx(overrides?: Partial<WorkflowContext>): WorkflowContext {
  return { source: '/test.fountain', ...overrides };
}

describe('WorkflowHookRegistry', () => {
  describe('register and execute', () => {
    it('should register and execute a hook handler', async () => {
      const registry = new WorkflowHookRegistry();
      const handler = vi.fn().mockImplementation(async (ctx: WorkflowContext) => ({
        ...ctx,
        globalStyle: 'anime',
      }));
