 ...
 * - Emits WorkflowEvent for each lifecycle point
 */
...
import type { WorkflowHookRegistry } from './hook-registry';
...
export class WorkflowExecutor implements IWorkflowExecutor {
  constructor(private readonly hookRegistry?: WorkflowHookRegistry) {}
  ...
  execute(
    ...
    const events: AsyncIterable<WorkflowEvent> = {
        ...
        return {
          async next(): Promise<IteratorResult<WorkflowEvent>> {
            ...
            while (true) {
              ...
              if (pipelineDone) {
                return { value: undefined as unknown as WorkflowEvent, done: true };
              }
    ...
    const result = (async (): Promise<WorkflowContext> => {
 ...
 */
export function createWorkflowExecutor(hookRegistry?: WorkflowHookRegistry): IWorkflowExecutor {
  return new WorkflowExecutor(hookRegistry);
}
