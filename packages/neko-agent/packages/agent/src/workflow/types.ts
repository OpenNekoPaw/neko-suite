 ...
 */
export interface IWorkflowStage<TCtx extends WorkflowContext = WorkflowContext> {
  /** Unique stage name */
...
export interface EvalResult {
  ...
  adjustments?: Partial<WorkflowContext>;
}
 ...
 */
export interface WorkflowReferenceChainEntry {
...
export interface WorkflowContext {
   ...
   */
  referenceChain?: readonly WorkflowReferenceChainEntry[];

  // — Phase 6.3 Lossless Upgrade provenance —
  /**
   * NkPlan id that orchestrated this run.  Set by orchestrator-bootstrap
   * when `startRoutedWorkflow` dispatches a routed pipeline.  Stamped
   * onto:
...
export interface WorkflowHandle {
  ...
  readonly result: Promise<WorkflowContext>;
}
...
export interface WorkflowRunReport {
  /** Pipeline execution ID (matches WorkflowHandle.id) */
  id: string;
