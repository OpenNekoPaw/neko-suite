...
When the current chat model cannot inspect the media directly, or when the configured media perception model is different from the chat model, use the runtime perception interface such as \`perception.perceive\` with a stable generated asset or resource ref. Do not infer visual quality, OCR, composition, style, or defects from a generation prompt, task id, file path, or thumbnail label alone.
...
当当前 chat 模型不能直接检查媒体，或配置的媒体感知模型与 chat 模型不一致时，使用运行时感知接口，例如对稳定的 generated asset 或 resource ref 调用 \`perception.perceive\`。不要仅根据生成提示词、task id、文件路径或缩略图标签推断视觉质量、OCR、构图、风格或瑕疵。
