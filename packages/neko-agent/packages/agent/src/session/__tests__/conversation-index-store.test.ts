...
describe('ConversationIndexStore', () => {
  ...
  it('resolves legacy aliases to canonical conversation IDs', async () => {
    const indexPath = '/tmp/conversations-index.json';
    const { files, readFile, writeFile, exists } = createMemoryFs();
    const store = new ConversationIndexStore({
      filePath: indexPath,
    ...
    await store.upsert({
      conversationId: 'abcd1234-01HZXCVBNM1234567890ABCDE',
      title: 'Migrated',
      workDir: '/workspace/demo',
      createdAt: 1000,
      updatedAt: 2000,
    ...
    await store.registerAlias('conv-legacy-id', 'abcd1234-01HZXCVBNM1234567890ABCDE');
    await store.flush();

    expect(await store.resolveId('conv-legacy-id')).toBe('abcd1234-01HZXCVBNM1234567890ABCDE');

    const indexData = JSON.parse(files.get(indexPath) ?? '{}') as {
      aliases?: Record<string, string>;
    };
    expect(indexData.aliases).toEqual({
      'conv-legacy-id': 'abcd1234-01HZXCVBNM1234567890ABCDE',
    });
  });

  it('removes aliases when the target conversation is deleted', async () => {
    const indexPath = '/tmp/conversations-index.json';
    const { readFile, writeFile, exists } = createMemoryFs({
      [indexPath]: JSON.stringify({
        version: 1,
        workspaces: {
          '/workspace/demo': ['abcd1234-01HZXCVBNM1234567890ABCDE'],
        },
        conversations: {
          'abcd1234-01HZXCVBNM1234567890ABCDE': {
            conversationId: 'abcd1234-01HZXCVBNM1234567890ABCDE',
            title: 'Migrated',
            workDir: '/workspace/demo',
            createdAt: 1000,
    ...
    await store.delete('abcd1234-01HZXCVBNM1234567890ABCDE');

    expect(await store.resolveId('conv-legacy-id')).toBeUndefined();
  });
});
