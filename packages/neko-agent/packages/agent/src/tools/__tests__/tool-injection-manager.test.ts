...
describe('ToolInjectionManager', () => {
  ...
  // -----------------------------------------------------------------------
  // Ablation: allowDynamicActivation + injectionMode (P1-A)
  // -----------------------------------------------------------------------

  describe('ablation: allowDynamicActivation=false', () => {
    it('activateToolSet silently drops when disabled', () => {
      const reg = makeMockCategoryRegistry();
      const provider = makeMockToolProvider([], []);
      const m = new ToolInjectionManager(reg, provider, { allowDynamicActivation: false });

      m.activateToolSet('some-skill');

      expect(m.getActiveToolSets()).toEqual([]);
    });

    it('activateToolSet emits no skill_activated event when disabled', () => {
      const reg = makeMockCategoryRegistry();
      const m = new ToolInjectionManager(reg, undefined, { allowDynamicActivation: false });
      const listener = vi.fn();
      m.addEventListener(listener);

      m.activateToolSet('any');

      expect(listener).not.toHaveBeenCalled();
    });

    it('default (undefined / true) preserves current activation behavior', () => {
      const reg = makeMockCategoryRegistry();
      const m = new ToolInjectionManager(reg);
      m.activateToolSet('skill-a');
      expect(m.getActiveToolSets()).toEqual(['skill-a']);
    });
  });

  describe("ablation: injectionMode='always-only'", () => {
    it("getToolsForTurn returns only always-layer tools when mode is 'always-only'", () => {
      const reg = makeMockCategoryRegistry();
      const provider = makeMockToolProvider(['dynamic-tool-1'], ['dynamic-tool-2']);
      const m = new ToolInjectionManager(reg, provider, { injectionMode: 'always-only' });

      m.activateToolSet('some-skill'); // would normally add dynamic tools
      const tools = m.getToolsForTurn('input');

      // No dynamic tools should appear
      expect(tools).not.toContain('dynamic-tool-1');
      expect(tools).not.toContain('dynamic-tool-2');
      // CORE_TOOLS still present (they're in the always layer)
      expect(tools).toEqual(expect.arrayContaining([...CORE_TOOLS]));
    });

    it("default mode ('always+dynamic') still includes dynamic tools", () => {
      const reg = makeMockCategoryRegistry();
      const provider = makeMockToolProvider(['dyn-1'], ['dyn-2']);
      const m = new ToolInjectionManager(reg, provider);

      m.activateToolSet('some-skill');
      const tools = m.getToolsForTurn('input');

      expect(tools).toContain('dyn-1');
      expect(tools).toContain('dyn-2');
    });
  });
});
