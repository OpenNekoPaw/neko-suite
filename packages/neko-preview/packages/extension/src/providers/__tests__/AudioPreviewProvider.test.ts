...
function createMockPreviewService(overrides: Record<string, unknown> = {}): PreviewService {
	return {
		...
		getStreamWebSocketUrl: vi.fn(
			(id: string) => `ws://127.0.0.1:8080/v1/streams/${id}`,
		),
		getWaveform: vi.fn(),
		dispatch: vi.fn(),
		dispose: vi.fn(),
...
describe('AudioPreviewProvider', () => {
	...
	describe('resolveCustomEditor()', () => {
		...
		it('should use audio entry for webview HTML', async () => {
			...
			expect(getWebviewHtml).toHaveBeenCalledWith(
				expect.objectContaining({ entry: 'audio' }),
			);
		});
	...
	describe('message handling', () => {
		...
		it('should handle "preview:play" by dispatching audio stream', async () => {
			const { mockService, panel, messageHandler } =
				await setupWithMessageHandler();
		...
		it('should handle "preview:pause" message', async () => {
			const { mockService, messageHandler } =
				await setupWithMessageHandler();
			...
			expect(mockService.pauseStreams).toHaveBeenCalledWith(
				null,
				'audio-stream-1',
			);
		});
		...
		it('should handle "preview:resume" message', async () => {
			...
			expect(mockService.resumeStreams).toHaveBeenCalledWith(
				null,
				'audio-stream-1',
			);
		});
		...
		it('should handle "preview:stop" message', async () => {
			...
			expect(mockService.stopStreams).toHaveBeenCalledWith(
				null,
				'audio-stream-1',
			);
		});
		...
		it('should handle "preview:seek" message with valid time', async () => {
			...
			expect(mockService.seekStreams).toHaveBeenCalledWith(
				null,
				'audio-stream-1',
				60.0,
			);
		...
		it('should handle "preview:speed" message', async () => {
			...
			expect(mockService.setStreamSpeed).toHaveBeenCalledWith(
				null,
				'audio-stream-1',
				0.5,
			);
