...
impl MicCaptureService {
    ...
    pub fn start_capture(
        ...
        let device = if let Some(id) = device_id {
            host.input_devices()
                .map_err(|e| Error::Other(format!("Failed to enumerate devices: {e}")))?
                .find(|d| d.name().map(|n| n == id).unwrap_or(false))
                .ok_or_else(|| Error::Other(format!("Device not found: {id}")))?
        } else {
            host.default_input_device()
                .ok_or_else(|| Error::Other("No default input device".to_string()))?
        };
        ...
        let stream = device
            ...
            .map_err(|e| Error::Other(format!("Failed to build input stream: {e}")))?;

        stream
            .play()
            .map_err(|e| Error::Other(format!("Failed to start stream: {e}")))?;
    ...
    pub async fn stop_capture(
        ...
        let recording = self
            ...
            .map_err(|_| Error::Other("Lock poisoned".to_string()))?
            .remove(stream_id)
            .ok_or_else(|| Error::Other(format!("Recording not found: {stream_id}")))?;
