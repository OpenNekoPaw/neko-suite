...
# Prerequisites: Server must be running (neko-engine serve --port <port>)
# =============================================================================
...
check_server() {
    ...
    if curl -s --connect-timeout 5 "$BASE_URL/health" > /dev/null 2>&1; then
        ...
        echo "Please start the server first: neko-engine serve --port $PORT"
        exit 1
    fi
}
...
get_server_cpu_usage() {
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ps aux | grep "neko-engine" | grep -v grep | head -1 | awk '{print $3}'
    else
        ps aux | grep "neko-engine" | grep -v grep | head -1 | awk '{print $3}'
    fi
}
