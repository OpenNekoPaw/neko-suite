...
```bash
# 测试：端口被占用时的行为
# 先启动一个服务占用端口，再启动 neko-engine
neko-engine serve --port 8765
...
```bash
# H.265 编码
neko-engine export --jvi-file test.jvi --output output_h265.mp4 --codec h265

# VP9 编码
neko-engine export --jvi-file test.jvi --output output_vp9.webm --codec vp9

# ProRes 编码
neko-engine export --jvi-file test.jvi --output output_prores.mov --codec prores
```
...
```bash
# 测试：不存在的输入文件
neko-engine export --jvi-file nonexistent.jvi --output output.mp4

# 预期：报错提示文件不存在

# 测试：无效的编码器
neko-engine export --jvi-file test.jvi --output output.mp4 --codec invalid
...
```bash
# 测试：探测视频文件（文本格式）
neko-engine probe --input video.mp4
...
```bash
# 测试：探测视频文件（JSON 格式）
neko-engine probe --input video.mp4 --format json
...
```bash
# 测试：探测音频文件
neko-engine probe --input audio.mp3
...
```bash
# 测试：探测带字幕的视频
neko-engine probe --input video_with_subs.mkv --format json
...
```bash
# 测试：提取第 0 秒（首帧）
neko-engine extract --input video.mp4 --output first.jpg --time 0.0

# 测试：提取超出时长的时间点
neko-engine extract --input video.mp4 --output last.jpg --time 9999.0
...
**包名**: `@neko/media-processor-rs`

### 1. MediaProcessor 创建测试

```typescript
import { MediaProcessor } from '@neko/media-processor-rs';
...
```typescript
import { probeMedia } from '@neko/media-processor-rs';
...
# 提取帧
neko-engine extract --input output.mp4 --output frame_check.jpg --time 5.0
...
# 获取视频时长
DURATION=$(neko-engine probe --input "$OUTPUT_FILE" --format json | jq '.duration')
...
```bash
#!/bin/bash
# 监控 neko-engine 进程的 CPU 使用率

PROCESS_NAME="neko-engine"
