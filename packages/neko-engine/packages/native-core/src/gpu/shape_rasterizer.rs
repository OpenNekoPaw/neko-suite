...
//! Pipeline:
//! ```text
//! ShapeElementData → [tiny-skia path + fill/stroke/shadow] → RGBA Vec<u8>
...
///
/// Rasterizes `ShapeElementData` at the output canvas size, then uploads the
...
impl ShapeRasterizer {
    ...
    fn build_path(&self, data: &ShapeElementData, w: u32, h: u32) -> Option<Path> {
        let fw = w as f32;
        let fh = h as f32;
