    ...
    async fn handle(
        ...
        match action {
            ...
            "capture" => {
                ...
                let (res_id, file_path) = resolve_resource(&self.resource_registry, resource_id, opts.source.as_deref()).await?;
                ...
                // Capture frame
                let frame_data = self.video_service.capture(&file_path, opts.time, capture_opts).await?;
