...
export interface NativeEngineType {
	...
	dispatchAction(group: string, action: string, id: string | null, options: string | null): Promise<string>;
	hasGpu(): boolean;
	groups(): string[];
...
export class NativeMediaEngine implements IMediaEngine {
	...
	async initialize(_options?: MediaEngineInitOptions): Promise<void> {
		...
		try {
			...
			console.log(`[NativeMediaEngine] NativeEngine created (GPU: ${hasGpu ? 'enabled' : 'disabled'})`);
