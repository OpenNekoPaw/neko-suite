...
function IconSpectrum() {
  return (
    <svg
      ...
      className="w-4 h-4"
    >
      <rect x="2"  y="12" width="2.5" height="6"  rx="1" fill="currentColor" stroke="none" opacity="0.9" />
      <rect x="6"  y="7"  width="2.5" height="11" rx="1" fill="currentColor" stroke="none" opacity="0.9" />
      <rect x="10" y="4"  width="2.5" height="14" rx="1" fill="currentColor" stroke="none" opacity="0.9" />
      <rect x="14" y="9"  width="2.5" height="9"  rx="1" fill="currentColor" stroke="none" opacity="0.9" />
    </svg>
  );
}
...
export function Toolbar() {
  ...
  const handleAnalyzeLoudness = useCallback(() => { postMessage({ type: 'editor:analyzeLoudness' }); }, []);
  const handleDetectSilence   = useCallback(() => { postMessage({ type: 'editor:detectSilence' });   }, []);
  const handleDenoise         = useCallback(() => { postMessage({ type: 'editor:denoise' });          }, []);
  const handleNormalize       = useCallback(() => { postMessage({ type: 'editor:normalize' });        }, []);

  return (
    <VerticalToolbar>
      {/* View toggles */}
      <ToolbarButton icon={<IconSpectrum />} title={t('audio.spectrum.toggle')} active={showSpectrum} onClick={toggleSpectrum} />

      <ToolbarSeparator />

      {/* Panel toggles */}
      <ToolbarButton icon={<IconEffects />}   title={t('audio.effects.title')}   active={activeSidePanel === 'effects'}   onClick={handleTogglePanel('effects')} />
      <ToolbarButton icon={<IconRecording />} title={t('audio.recording.title')} active={activeSidePanel === 'recording'} onClick={handleTogglePanel('recording')} />
      <ToolbarButton icon={<IconExport />}    title={t('audio.export.toggle')}   active={activeSidePanel === 'export'}    onClick={handleTogglePanel('export')} />

      <ToolbarSeparator />

      {/* Actions */}
      <ToolbarButton icon={<IconLoudness />}  title={t('audio.analysis.loudness')} onClick={handleAnalyzeLoudness} />
      <ToolbarButton icon={<IconSilence />}   title={t('audio.analysis.silence')}  onClick={handleDetectSilence} />
      <ToolbarButton icon={<IconDenoise />}   title={t('audio.analysis.denoise')}  onClick={handleDenoise} />
      <ToolbarButton icon={<IconNormalize />} title={t('audio.analysis.normalize')} onClick={handleNormalize} />
    </VerticalToolbar>
  );
}
