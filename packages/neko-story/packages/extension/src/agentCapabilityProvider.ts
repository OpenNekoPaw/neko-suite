    ...
    return [
      {
        ...
        content: [
          ...
          '',
          'Standard Fountain with CJK extensions. English and CJK coexist.',
          '',
          '### Scene headings',
          '`INT./EXT./EST./I/E. LOCATION - TIME` | `.` forced heading | Scene number: `INT. OFFICE - DAY #1#`',
          'CJK: `内景`/`內景` (INT), `外景` (EXT), `内外景`/`內外景` (INT/EXT)',
          'Time: DAY/NIGHT/MORNING/EVENING/DAWN/DUSK/LATER/CONTINUOUS | 日/夜/黄昏/黎明/清晨/傍晚/稍后/连续',
          '',
          '### Characters & dialogue',
          'ALL CAPS (English) or CJK name (1-10 chars). `@` forced: `@McAvoy` / `@角色名`',
          'Extension: `BOB (V.O.)` / `张三丰（画外音）`. Dual dialogue: `ALICE ^`',
          'Blank line before, non-blank after. Parentheticals: `(whispering)` / `（低声地）`',
          '',
          '### Transitions',
          'English: `CUT TO:` `FADE TO:` `DISSOLVE TO:`. CJK: `切至：` `淡入：` `淡出：` `叠化：`',
          '`>` forced: `> FADE TO BLACK`',
          '',
          '### Other elements',
          'Action: plain paragraphs | Section: `# Act One` | Synopsis: `= summary`',
          'Centered: `> THE END <` | Lyrics: `~text` | Page break: `===`',
          'Title page: `Title: My Script` / `标题：我的剧本`',
          '',
          '### Notes & directives',
          '`[[free text note]]` — editorial comment (invisible to renderers)',
          '`[[KEY: value]]` — structured directive. Categories:',
          '',
          '| Category | Keys | Examples |',
          '|----------|------|---------|',
          '| Asset | IMAGE, VIDEO, AUDIO, ASSET | `[[IMAGE: hero-portrait.png]]` |',
          '| Metadata | MOOD, MUSIC, VFX, SFX, DURATION | `[[MOOD: tense]]` `[[DURATION: 30s]]` |',
          '| Camera | SHOT, ANGLE, MOVEMENT | `[[SHOT: close-up]]` `[[ANGLE: low]]` |',
          '| AI | PROMPT, STYLE, REF | `[[PROMPT: cyberpunk café, neon rain]]` |',
          '',
          'DURATION format: `30s` / `1m30s` / `2m`. Overrides heuristic scene duration.',
          'Line comment: `// text` (parser-level, not rendered).',
          'Block note: `/* ... */` (multi-line).',
          '',
          '### Priority',
          '`@`/`.`/`>` forced markers override auto-detection. CJK is additive.',
        ].join('\n'),
        priority: 70,
      },
    ];
