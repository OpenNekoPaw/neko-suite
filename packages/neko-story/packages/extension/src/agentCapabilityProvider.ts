    ...
    return [
      {
        ...
        content: [
          ...
          '',
          'neko-story `.fountain` files support standard Fountain syntax with',
          'CJK extensions. English and CJK coexist in the same file.',
          '',
          '### Scene headings',
          '',
          '| Syntax | Example |',
          '|--------|---------|',
          '| `INT./EXT. LOCATION - TIME` | `INT. COFFEE SHOP - DAY` |',
          '| `内景`/`內景` (= INT) | `内景 咖啡厅 - 日` |',
          '| `外景` (= EXT) | `外景 公园 - 夜` |',
          '| `内外景`/`內外景` (= INT/EXT) | `内外景 阳台 - 黄昏` |',
          '| `.` forced heading | `.A darkness surrounds` |',
          '',
          'Time-of-day: `DAY` `NIGHT` `MORNING` `EVENING` `DAWN` `DUSK`',
          '`LATER` `CONTINUOUS` `MOMENTS LATER` `SAME TIME`',
          '`日` `夜` `黄昏` `黎明` `清晨` `傍晚` `稍后` `连续`',
          '',
          '### Characters',
          '',
          '| Syntax | Example |',
          '|--------|---------|',
          '| ALL CAPS (English) | `ALICE` |',
          '| CJK name (1-10 chars) | `李明` |',
          '| With extension | `BOB (V.O.)` / `张三丰（画外音）` |',
          '| Dual dialogue | `ALICE ^` / `李明 ^` |',
          '| `@` forced character | `@McAvoy` / `@角色名` |',
          '',
          'Character detection requires: blank line before, non-blank line after.',
          'CJK names must not contain sentence punctuation (`。！？…；，、：`).',
          '',
          '### Dialogue & parentheticals',
          '',
          'Regular text after a character line. Parentheticals:',
          '`(whispering)` or `（低声地）` (full-width parens).',
          '',
          '### Transitions',
          '',
          '| Syntax | Example |',
          '|--------|---------|',
          '| English (ends with `TO:`) | `CUT TO:` `FADE TO:` `DISSOLVE TO:` |',
          '| CJK keyword + colon | `切至：` `淡入：` `淡出：` `叠化：` `化入：` `化出：` `跳切：` `交叉剪辑：` |',
          '| `>` forced transition | `> FADE TO BLACK` |',
          '',
          '### Other elements',
          '',
          '- Action: plain paragraphs',
          '- Section: `# Act One` / `## Scene 1`',
          '- Synopsis: `= A brief summary`',
          '- Centered: `> THE END <`',
          '- Note: `[[editorial note]]` / `/* block note */`',
          '- Lyrics: `~Singing in the rain`',
          '- Page break: `===`',
          '- Title page: `Title: My Script` or `标题：我的剧本` (full-width colon accepted)',
          '',
          '### Priority',
          '',
          '`@` / `.` / `>` forced markers always take priority over auto-detection.',
          'CJK native syntax is an additional recognition path, not a replacement.',
        ].join('\n'),
        priority: 70,
      },
    ];
