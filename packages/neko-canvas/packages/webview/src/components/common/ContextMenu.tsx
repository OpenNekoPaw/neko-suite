...
export function buildCanvasMenuItems(ctx: CanvasMenuContext): MenuEntry[] {
  return [
    ...
    { separator: true },
    { label: t('menu.paste'),        icon: '📋', shortcut: '⌘V',  onClick: () => ctx.onPaste?.(),        disabled: !ctx.canPaste },
    { label: t('menu.pasteInPlace'), icon: '📌', shortcut: '⇧⌘V', onClick: () => ctx.onPasteInPlace?.(), disabled: !ctx.canPaste },
    { separator: true },
    { label: t('menu.undo'), icon: '↩', shortcut: '⌘Z',  onClick: () => ctx.onUndo?.(), disabled: !ctx.canUndo },
    { label: t('menu.redo'), icon: '↪', shortcut: '⇧⌘Z', onClick: () => ctx.onRedo?.(), disabled: !ctx.canRedo },
    { separator: true },
...
export function buildNodeMenuItems(ctx: CanvasMenuContext): MenuEntry[] {
  return [
    ...
    { separator: true },
    { label: t('menu.group'),   icon: '📁', shortcut: '⌘G',  onClick: () => ctx.onGroup?.(),   disabled: !ctx.canGroup },
    { label: t('menu.ungroup'), icon: '📂', shortcut: '⇧⌘G', onClick: () => ctx.onUngroup?.(), disabled: !ctx.canUngroup },
    { separator: true },
