...
const CANVAS_AUTHORING_SKILL_EN = [
  ...
  '',
  '## Recipes',
  '- Storyboard creation is a Canvas recipe, not a separate Agent workflow. Completed storyboard creative tables should become Canvas scene and shot nodes when the user asks for production storyboard delivery.',
  ...
  '- "Send as Markdown" means Markdown is the source format/transport, not automatically a review-only request.',
  '- Use review-only Canvas ingestion only when the user explicitly asks for a review table or draft instead of production scene/shot nodes.',
...
const CANVAS_AUTHORING_SKILL_ZH = [
  ...
  '',
  '## Recipes',
  '- 分镜创建是 Canvas recipe，不是单独的 Agent 工作流。用户要求生产级分镜交付时，完成的分镜 creative table 应成为 Canvas scene 和 shot 节点。',
  ...
  '- “作为 Markdown/Markdown 发送”表示 Markdown 是来源格式/传输格式，不自动等于 review-only 请求。',
  '- 只有用户明确要求 review table 或 draft，而不是生产级 scene/shot 节点时，才使用 review-only Canvas 摄入。',
