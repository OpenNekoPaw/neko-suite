...
const CANVAS_AUTHORING_SKILL_EN = [
  ...
  '- Inspect the Canvas-owned authoring catalog when you need supported node types, presets, containers, connections, fields, operations, recipes, or prompt support.',
  '- Inspect the active Canvas context before choosing insertion points, selected nodes, focused containers, or targetable fields.',
  '- Inspect existing relationships before creating or changing node connections.',
  ...
  '',
  '## Recipes',
  '- Unspecified Storyboard exploration, analysis, planning, alternatives, and first drafts remain ordinary Markdown document content. Do not create scene, shot, or specialized table nodes for that intent.',
  '- Structured Storyboard creation is a professional Canvas recipe that requires explicit user intent. Only then may validated canonical content become scene and shot nodes.',
  '- Ad hoc single-scene structures that are not Markdown storyboard handoffs should use Canvas composite authoring with scene and shot children.',
  '- A single stable asset or reference should become a media node.',
  '- A completed canonical source Storyboard must already exist before explicit professional structured authoring.',
  '- A source Storyboard used for structured authoring must be visible as an assistant Markdown block or UI handoff source. If it only exists inside tool arguments, output the Markdown and stop instead of calling structured Canvas tools.',
  '- Do not use Canvas handoff to skip comic/page visual analysis or storyboard table generation.',
  '- Markdown is the default unspecified Storyboard result. Do not infer structured creation from the presence of a table or Storyboard terminology.',
  '- Use structured scene/shot authoring only when the user explicitly asks to create or update professional structured Storyboard content.',
  '- If production storyboard delivery is unavailable or blocked, report the Canvas diagnostics and repair the table, approval, target, or resource refs before retrying.',
  ...
  '- Never bind resources by runtime display identifiers, row order, display label, or cache-derived paths.',
  '',
...
const CANVAS_AUTHORING_SKILL_ZH = [
  ...
  '',
  '## Query Before Mutation',
  '- 需要节点类型、预设、容器、连接、字段、操作、recipe 或 prompt 支持时，先查看 Canvas 拥有的 authoring catalog。',
  '- 选择插入点、选中节点、焦点容器或目标字段前，先查看当前 Canvas context。',
  '- 创建或修改节点连接前，先查看已有关系。',
  '- 把 Canvas diagnostics 当作修复指令。只有修正 target、preset、field、resource ref 或审批状态后才重试。',
  '',
  '## Recipes',
  '- 未指定结构化创作的分镜探索、分析、规划、方案和初稿保持为普通 Markdown 文档内容；不得因此创建 scene、shot 或专用 table 节点。',
  '- 结构化分镜创建是专业 Canvas recipe，必须有用户明确意图；只有此时，经过验证的 canonical 内容才可以成为 scene 和 shot 节点。',
  '- 对于不是 Markdown 分镜 handoff 的临时单场景结构，使用 Canvas composite authoring 表达 scene 和 shot children。',
  '- 单个稳定素材或引用应成为 media 节点。',
  '- 明确执行专业结构化 authoring 前，必须已经存在完成的 canonical 来源 Storyboard。',
  '- 用于结构化 authoring 的来源 Storyboard 必须是可见 assistant Markdown 块或 UI handoff 来源；如果只存在于工具参数中，先输出 Markdown 并停止，不要调用结构化 Canvas 工具。',
  '- 不要用 Canvas handoff 跳过漫画/页面视觉分析或分镜表生成。',
  '- Markdown 是未指定分镜请求的默认结果；不能因为内容包含表格或 Storyboard 术语就推断为结构化创建。',
  '- 只有用户明确要求创建或更新专业结构化 Storyboard 时，才使用 scene/shot 结构化 authoring。',
  '- 如果生产级分镜交付不可用或被阻塞，报告 Canvas diagnostics，并修复表格、审批、目标或资源绑定后再重试。',
  ...
  '- 不要通过运行时显示身份、行号、显示标签或 cache 派生路径绑定资源。',
  '',
  '## Semantic Storyboard Authoring',
