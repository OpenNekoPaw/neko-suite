...
const logger = getLogger('MediaDiffMessageHandler');
...
export class MediaDiffMessageHandler implements vscode.Disposable {
  ...
  private runAnalysisPipeline(
    ...
    const run = async () => {
      try {
        ...
        parallelTasks.push(
          this.sendVisualizationData({ mediaType, similarity: -1 } as DiffResult, ref).catch(
            (err) => logger.warn('Frame extraction failed:', err),
          ),
        );
        ...
        if (this.timeRange.startTime !== undefined)
          diffOptions.startTime = this.timeRange.startTime;
  ...
  private runLocalAnalysisPipeline(
    ...
    const run = async () => {
      try {
        ...
        parallelTasks.push(
          this.sendVisualizationDataForLocal({ mediaType, similarity: -1 } as DiffResult).catch(
        ...
        if (this.timeRange.startTime !== undefined)
          localDiffOptions.startTime = this.timeRange.startTime;
    ...
    try {
      switch (type) {
        ...
        case 'mediaDiff:getFrame':
          ...
          await this.handleGetFrame(message.payload.time, message.payload.version, requestId);
          break;

        case 'mediaDiff:inspectElement':
          // Lazy content diff: extract thumbnail for a media element
          await this.handleInspectElement(message.payload.src, requestId);
          break;
        ...
        case 'mediaDiff:streamControl':
          await this.handleStreamControl(message.payload.action, message.payload, requestId);
          break;
        ...
        case 'mediaDiff:audioStreamControl':
          await this.handleAudioStreamControl(message.payload.action, message.payload, requestId);
          break;
      }
    } catch (error) {
      this.sendMessage({
  ...
  private async sendImageDataForLocal(): Promise<void> {
    ...
    try {
      const versions = await this.diffService.getLocalFileVersions(this.fileUri, this.previousUri);
   ...
   */
  private async handleInspectElement(src: string, requestId?: string): Promise<void> {
    if (!src) return;
  ...
  private async handleStartStreaming(requestId?: string): Promise<void> {
    try {
      ...
      const videoDetails =
        this.lastDiffResult?.mediaType === 'video'
      ...
      try {
        ...
        if (curAudioResult.status === 'fulfilled') {
          ...
          logger.debug('Current audio stream created:', this.currentAudioStreamId);
        } else {
        ...
        if (prevAudioResult.status === 'fulfilled') {
          ...
          logger.debug('Previous audio stream created:', this.previousAudioStreamId);
        } else {
  ...
  private async handleStartAudioStreaming(requestId?: string): Promise<void> {
    try {
      ...
      const audioDetails =
        this.lastDiffResult?.mediaType === 'audio'
  ...
  private async handleAudioStreamControl(
    ...
    const allStreams = [this.currentAudioOnlyStreamId, this.previousAudioOnlyStreamId].filter(
