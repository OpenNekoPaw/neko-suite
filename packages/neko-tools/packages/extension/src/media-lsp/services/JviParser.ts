/**
 * JVI Parser — Parse .jvi JSON into JviParsedProject with source positions.
 *
...
function parseElement(
	...
	return {
		...
		linkedAudioId:
			linkedAudioIdNode?.type === 'string'
				? (linkedAudioIdNode.value as string)
				: undefined,
		linkedAudioIdRange: linkedAudioIdNode
			? nodeToRange(index, linkedAudioIdNode)
			: undefined,
		linkedVideoId:
			linkedVideoIdNode?.type === 'string'
				? (linkedVideoIdNode.value as string)
				: undefined,
		linkedVideoIdRange: linkedVideoIdNode
			? nodeToRange(index, linkedVideoIdNode)
			: undefined,
...
function parseTrack(index: LineIndex, trackNode: Node): JviParsedTrack | null {
	...
	return {
		...
		nameRange: nameNode
			? nodeToRange(index, nameNode)
			: nodeToRange(index, trackNode),
	};
}
...
export function parseJviDocument(text: string): JviParsedProject {
	...
	const width =
		resNode?.type === 'object' ? getNumberValue(resNode, 'width') : 0;
	const height =
		resNode?.type === 'object' ? getNumberValue(resNode, 'height') : 0;
 ...
 */
export function findElementIdRange(
	text: string,
	elementId: string,
): JviRange | null {
	const root = parseTree(text);
