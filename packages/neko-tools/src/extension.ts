...
export function activate(context: vscode.ExtensionContext) {
    ...
    async (entityId: string): Promise<AssetEntity | null> => {
      try {
        const entities: AssetEntity[] =
          (await vscode.commands.executeCommand('neko.assets.getAllEntities')) ?? [];
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('neko.tools.compareAssetVariants', async () => {
      ...
      const multiVariantEntities = entities.filter((e) => e.variants.length >= 2);
      if (multiVariantEntities.length === 0) {
        vscode.window.showInformationMessage('No entities with multiple variants found.');
        return;
      }
      ...
      const remainingVariants = entity.variants.filter((v) => v.id !== variantAPick.variant.id);
          ...
          for (const variant of entity.variants) {
            for (const file of variant.files) {
              if (uri.fsPath.endsWith(file.path) || file.path === uri.fsPath) {
                ...
                if (meta.fileSize)
                  lines.push(`Size: ${(meta.fileSize / (1024 * 1024)).toFixed(1)} MB`);
