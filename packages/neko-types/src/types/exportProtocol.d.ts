 ...
 * 导出任务状态
 */
export type ExportJobState = 'pending' | 'initializing' | 'rendering' | 'encoding' | 'muxing' | 'finalizing' | 'completed' | 'cancelled' | 'error';
/**
 * 导出任务状态信息
 */
 ...
 * 所有导出响应类型的联合
 */
export type ExportResponse = ExportStartResponse | ExportProgressResponse | ExportCompleteResponse | ExportCancelledResponse | ExportErrorResponse | ExportStatusResponse;
/**
 * 导出协议版本
 */
export declare const EXPORT_PROTOCOL_VERSION = "1.0.0";
/**
 * 导出请求超时时间（毫秒）
 * 注意：这是启动导出的超时，不是导出完成的超时
 */
export declare const EXPORT_START_TIMEOUT = 30000;
/**
 * 最大并发导出任务数
 */
export declare const MAX_CONCURRENT_EXPORTS = 1;
/**
 * 导出能力检测结果
 */
 ...
 * FFmpeg 支持的特效映射
 */
export declare const FFMPEG_SUPPORTED_FEATURES: {
    /** 变换 */
    readonly transforms: readonly ["position", "scale", "rotation"];
    /** 混合模式（仅支持部分） */
    readonly blendModes: readonly ["normal", "multiply", "screen", "overlay"];
    /** 颜色校正 */
    readonly colorCorrection: readonly ["brightness", "contrast", "saturation"];
    /** 转场（使用 xfade） */
    readonly transitions: readonly ["fade", "dissolve", "wipe", "slide"];
    /** 不支持的特效 */
    readonly unsupported: readonly ["custom-shader", "motion-blur", "complex-mask", "advanced-blend-modes"];
};
/**
 * 流式导出模式
 *
 ...
 */
export type StreamingExportMode = 'legacy' | 'streaming';
/**
 * 帧数据格式
 */
export type FrameDataFormat = 'rgba' | 'jpeg' | 'png';
/**
 ...
 * 默认流式导出配置
 */
export declare const DEFAULT_STREAMING_EXPORT_SETTINGS: StreamingExportSettings;
/**
 ...
 * 扩展的导出请求联合类型
 */
export type StreamingExportRequest = ExportRequest | StreamingExportInitRequest | StreamingExportPushFrameRequest | StreamingExportFinalizeRequest;
/**
 ...
 * 扩展的导出响应联合类型
 */
export type StreamingExportResponse = ExportResponse | StreamingExportInitResponse | StreamingExportFrameAckResponse | StreamingExportResumeResponse;
/**
 * 背压状态
 */
 ...
 * 流式导出任务扩展状态
 */
export type StreamingExportJobState = ExportJobState | 'streaming';
/**
 * 流式导出任务状态
 */
 ...
 * 流式导出协议版本
 */
export declare const STREAMING_EXPORT_PROTOCOL_VERSION = "2.0.0";
/**
 * 流式视频解码器状态
 */
export type StreamingDecoderState = 'idle' | 'starting' | 'decoding' | 'paused' | 'completed' | 'error';
/**
 * 视频源信息（用于初始化解码器）
 */
 ...
 * 流式解码请求联合类型
 */
export type StreamingDecodeRequest = StreamingDecodeInitRequest | StreamingDecodeGetFrameRequest | StreamingDecodeStopRequest;
/**
 * 流式解码初始化响应
 */
 ...
 * 流式解码响应联合类型
 */
export type StreamingDecodeResponse = StreamingDecodeInitResponse | StreamingDecodeFrameResponse | StreamingDecodeStopResponse;
//# sourceMappingURL=exportProtocol.d.ts.map