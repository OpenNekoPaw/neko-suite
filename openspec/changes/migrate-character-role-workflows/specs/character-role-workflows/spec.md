...
### Requirement: Core supports NPC test bench terminology as public contract
**Reason**: NPC terminology no longer matches the product actor relationships and makes automated validation look like a peer interaction mode.
**Migration**: Use `character-dialogue` for Agent-as-character testing and `embody-character` for user-as-character knowledge feedback. Use Skills for character validation and improvement.

### Requirement: Core exposes validate-character and improve-character workflow actions
**Reason**: Validation and improvement are automation strategies that should evolve through Skills, not Dashboard core action ids.
**Migration**: Remove `validate-character` and `improve-character` from shared action unions and command registration. Provide or update `character-validation` and `character-improvement` Skills that call core primitives.
