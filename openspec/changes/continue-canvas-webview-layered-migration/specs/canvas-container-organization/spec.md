...
#### Scenario: Legacy mirrors stay synchronized
- **WHEN** a container action changes Scene or Group membership during migration
- **THEN** the implementation updates the canonical container child IDs and the legacy mirror fields needed by old files and tools
...
- **THEN** the implementation delegates to the generic add-child container action and preserves legacy behavior
...
- **THEN** it determines that behavior from organization membership helpers and policy metadata rather than only checking Shot-specific legacy fields

#### Scenario: Clipboard remaps migrated containers
- **WHEN** a migrated container subtree is copied and pasted
- **THEN** the pasted nodes receive remapped canonical child IDs, parent IDs, and legacy mirrors consistently
