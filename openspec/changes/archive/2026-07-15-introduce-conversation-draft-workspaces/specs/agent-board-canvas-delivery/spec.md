...
The Agent delivery boundary SHALL automatically author creator-useful Markdown documents, selected file/reference inputs, completed image/audio/video outputs, reviewable variants, and creator-output task projections into the resolved Board Canvas through existing public Canvas authoring capabilities.
...
- **WHEN** an image, audio, or video task completes with a valid retained generated-output identity
- **THEN** Canvas receives a stable reference node/update for the frozen Board Canvas target
