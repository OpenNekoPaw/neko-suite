...
The Webview host-state slot retains a versioned, rebuildable draft snapshot keyed by the complete `tabId + conversationId` binding. It contains only structured-clone-safe composer text and future-turn configuration controls. Projection replicas, messages, attachments, context/file references, queued edits, focus, menus, diagnostics, media URLs, and runtime handles are excluded. Restore occurs only after the immutable Tab binding is reconciled; an owner mismatch or unknown schema version fails visibly. Store changes are coalesced and flushed on realm disposal so typing does not rewrite the complete host state for every keystroke.
