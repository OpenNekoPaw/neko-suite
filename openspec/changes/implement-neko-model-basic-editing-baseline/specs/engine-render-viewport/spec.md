...
The Engine render viewport contract SHALL support runtime interaction stream profile updates for high-frequency model editing without requiring a new stream descriptor. Interactive profile updates MAY adjust GOP, bitrate, backpressure hints, or frame pacing policy on the active stream, and MUST be time-bounded by idle timers or TTL.

#### Scenario: Interactive profile applies without stream restart
- **WHEN** scene-control receives a latest-only hot update with `streamProfile: 'interactive'`
- **THEN** Engine applies the interactive runtime profile to the active stream when supported
- **THEN** Engine does not require Webview to destroy and recreate the stream descriptor

#### Scenario: Interactive profile expires
- **WHEN** the interactive profile TTL or idle timer expires
- **THEN** Engine returns the active stream to the default runtime policy without invalidating the current stream connection
