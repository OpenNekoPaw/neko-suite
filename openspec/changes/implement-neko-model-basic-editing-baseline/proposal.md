...
The first implementation attempt for this change was rolled back after runtime testing showed visible camera/drag feedback could be delayed by seconds. The proposal remains valid, but all implementation must preserve the immediate interaction hot path: local UI intent first, Engine latest-only hot updates second, and video/frame metadata reconciliation asynchronously.
...
- If hot-path latency regresses, roll back the new interaction path before rolling back unrelated diagnostics or B0-B2 controls.
