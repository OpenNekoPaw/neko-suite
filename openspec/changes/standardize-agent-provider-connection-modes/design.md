...
- Make NewAPI-compatible gateway the MVP proxy/cloud path.
...
- Guarantee all NewAPI-compatible endpoints support all vendor-specific model parameters.
...
1. Add connection metadata instead of replacing `ProviderType`.
   ...
   - `protocolProfile`: `newapi-compatible`, `ollama`, `openai-chat`, `openai-responses`, `anthropic`, `google`, or other verified profiles.
...
2. Make the MVP provider set gateway/local-first.
   ...
   - `neko-gateway`: built-in NewAPI-compatible gateway profile.
   - `custom-newapi`: disabled custom NewAPI-compatible endpoint profile.
...
3. Keep NewAPI as the only proxy protocol implemented in MVP.
   ...
   OneAPI remains adapter-compatible where existing code already supports it, but product configuration defaults and docs focus on NewAPI-compatible behavior. OpenRouter/SubAPI/direct official providers stay as roadmap profiles until there are verified API mappings, model parameters, and tests.
...
- [Risk] NewAPI-compatible gateways differ in supported upstream models and parameters. → Mitigation: mark custom endpoints as `custom` support, avoid claiming broad upstream guarantees, and make unsupported generation/direct paths roadmap work.
