# agent-tool-result-backfill Specification

## Purpose
Define the shared tool result backfill contract and runtime closure that updates active stream state, persisted session/history, and Webview delivery after background media tasks complete. This capability ensures late-arriving assets, attachments, diagnostics, and perception cards become visible to both UI presentation and future LLM turns without persisting host-specific payloads.
