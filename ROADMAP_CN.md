...
| 模块             | 状态   | 进度 | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------------- | ------ | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **neko-engine**  | Alpha  | 98%  | GPU 渲染 + 编解码 + 导出 + HTTP/WS + 设备代理 + ONNX ML 推理 + 完整色彩/抠像管线 + 关键帧/动画混合 + 角色编辑 API + **并发保护 Semaphore(8/4/2) ✅** + **3D 渲染控制面 ✅** + 可配置日志级别                                                                                                                                                                                                                                                                        |
| **neko-agent**   | Alpha  | 99%  | **0 TODO**，1957+ 测试，300+ 文件；7 LLM + 10 媒体适配器 + MCP + Coordinator + SubAgent + Creative Memory + 质量评估 + Webview P0 完成 + **IDC 统一工作流 Phase A+B+闭环 ✅** + **多模态感知管线 ✅** + **富内容投递 ✅** + **运行时边界加固 ✅**（工作流评估框架、反馈闭环、边界适配器）+ **死代码清理 ✅**（13 个未使用桥接/路由移除）；剩余：MCP 重连退避 + P1 Zustand 迁移 + ask 模式解耦（[ADR](./docs/architecture/agent-unified-workflow.md)）               |
...
| **neko-canvas**  | Alpha  | 93%  | 13 种节点 + BatchGenerationScheduler + 7 MCP Tools；**P0 已全部收敛** ✅ + P1-1 CanvasEmbedNode + P1-4 NodeRendererRegistry + **NodeTypeDescriptor 注册表** ✅ + **Block + Container Phase 0/1 ✅** + **场景-镜头缩略图模式 + 管理 UI ✅**；剩余 P1 增强 + Phase 2 布局 packing（[ADR](./docs/architecture/canvas-role-boundary.md)）                                                                                                                              |
| **neko-preview** | Alpha  | 88%  | 6 种编辑器 + 瀑布流 + Content→Agent + **EPUB 大纲 TreeView ✅** + **全景图预览（进行中）**；一期剩余：FDX + 全景；二期：XLSX/PPTX（[ADR](./docs/architecture/adr-panoramic-image-preview.md)）                                                                                                                                                                                                                                                                       |
...
| **neko-types**   | Alpha  | 93%  | 共享类型 + 横切关注点统一 + Operations 类型安全 + **设备/追踪/预览契约 ✅**；后续 Layer 0 工具清理（`uniqueStrings` / `isRecord`）                                                                                                                                                                                                                                                                                                                                     |
| **neko-client**  | Alpha  | 82%  | H264/fMP4/PCM 流客户端 + EngineClient HTTP dispatch + **设备客户端类（进行中）** + **感知门面 ✅**                                                                                                                                                                                                                                                                                                                                                                    |
...
| 模块            | 状态  | 进度 | 说明                                                                                                            |
| --------------- | ----- | ---- | --------------------------------------------------------------------------------------------------------------- |
| **neko-audio**  | Alpha | 72%  | **0 TODO**，~9.1K LOC，78 测试；波形 + 频谱 + 12 种效果链 + 多轨骨架 + 麦克风；**缺失**：多轨 P0 UI（标记栏/片段拖拽/轨头/录制闭环）+ P1 混音台/自动化 + P2 总线/干声/连录                             |
...
- [ ] P2：批量候选对比器 + 更强审阅 UI
- [ ] P2：Block + Container 高级布局 packing（aspect-ratio-aware grid、policy-specific layout、`lockedChildIds` 避让、容器边界/绝对坐标语义保持）
...
**Skill prompt-chain 清理（2026-04-26）**：未上线前移除 Skill workflow DSL 字段的运行时契约与市场安装入口。新 Skill 写作统一使用正文 prompt-chain 章节 + 确定性 metadata 边界。
...
_最后更新：2026-05-07（设备管理进行中 + 全景预览进行中 + Agent 多模态感知管线 + 富内容投递 + 运行时边界加固 + neko-sketch PSD 导入 + neko-model 3D 修复 + 市场治理加固。）_
