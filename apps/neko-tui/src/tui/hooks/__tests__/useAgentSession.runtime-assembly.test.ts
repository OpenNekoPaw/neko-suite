...
const runtimeMocks = vi.hoisted(() => ({
  ...
  executeEventBatches: undefined as AgentEvent[][] | undefined,
...
beforeEach(async () => {
  ...
  runtimeMocks.executeEventBatches = undefined;
...
async function waitFor(predicate: () => boolean): Promise<void> {
  ...
  const runtimeError = currentRuntime().conversation.stores.agent.getState().error;
  throw new Error(
    `Timed out waiting for runtime assembly${runtimeError ? `: ${runtimeError.message}` : '.'}`,
  );
}
