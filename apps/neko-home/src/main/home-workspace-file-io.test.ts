...
import {
  createHomeProjectFileIoAdapter,
  resolveHomeWorkspacePath,
} from './home-workspace-file-io';

const packageRoot = resolve(dirname(fileURLToPath(import.meta.url)), '../..');

describe('home project file IO adapter', () => {
  it('reads workspace text files through host-authorized ProjectFileOps', async () => {
    const workspaceRoot = await createTempWorkspace();
    await writeFile(join(workspaceRoot, 'notes.md'), 'hello home');
    const adapter = createHomeProjectFileIoAdapter({ workspaceRoot });

    const content = await adapter.fileOps.readFile('notes.md');
    expect(new TextDecoder().decode(content)).toBe('hello home');
  ...
  it('truncates renderer text previews without changing durable file identity', async () => {
    ...
    const adapter = createHomeProjectFileIoAdapter({
      workspaceRoot,
      maxTextFileBytes: 3,
    });
  ...
  it('rejects project file paths outside the workspace for every file operation', async () => {
    ...
    expect(() => resolveHomeWorkspacePath(workspaceRoot, '../outside.txt')).toThrow(
      'Workspace file path is outside the workspace',
    );
    expect(() => resolveHomeWorkspacePath(workspaceRoot, outsidePath)).toThrow(
      'Workspace file path is outside the workspace',
    );
  ...
  it('keeps atomic rename overwrite behavior explicit for ProjectFileStore saves', async () => {
    ...
    if (!renameFile) {
      throw new Error('Home project file IO adapter must expose renameFile.');
    }
  ...
  it('keeps Home Agent file commands on the project-file IO boundary without editor IPC', () => {
    ...
    expect(mainSource).toContain('createHomeProjectFileIoAdapter');
    expect(mainSource).not.toContain('readWorkspaceFile');
    expect(mainSource).not.toContain('writeWorkspaceFile');
  });
});

async function createTempWorkspace(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'neko-home-project-file-io-'));
}
