...
const GENERATED_ARTIFACT_ROOT = ['neko', 'generated'] as const;
...
const WORKSPACE_SKILLS_PATH = ['.agents', 'skills'] as const;
const WORKSPACE_COMMANDS_PATH = ['.neko', 'commands'] as const;

export interface HomeResourceSurfaceInput {
  readonly workspaceRoot: string;
}

export interface HomeResourceSurfaceSnapshot {
...
export async function createHomeResourceSurfaces({
  workspaceRoot,
}: HomeResourceSurfaceInput): Promise<readonly ResourceSurfaceSnapshot[]> {
  const snapshot = await createHomeResourceSurfaceSnapshot({ workspaceRoot });
  return snapshot.resourceSurfaces;
}

export async function createHomeResourceSurfaceSnapshot({
  workspaceRoot,
}: HomeResourceSurfaceInput): Promise<HomeResourceSurfaceSnapshot> {
  const resourceSurfaces = await readHomeResourceSurfaces(workspaceRoot);
  return {
    resourceSurfaces,
...
async function readHomeResourceSurfaces(
  ...
  const nodesBySurface = new Map<HomeSurfaceId, readonly ResourceNode[]>([
    ['assets', assetNodes],
...
async function readAssetResourceNodes(workspaceRoot: string): Promise<readonly ResourceNode[]> {
  ...
  nodes.push(
    ...workspaceContent.mediaLibraries.map((library): ResourceNode => ({
      ...
      metadata: {
        ...
        tags: ['media-library', library.variable, ...(library.overridden ? ['overridden'] : [])],
      },
      badges: [
        {
...
async function readGenerationResourceNodes(
  workspaceRoot: string,
): Promise<readonly ResourceNode[]> {
  const [generatedArtifacts, dashboardActivity] = await Promise.all([
    readGeneratedArtifactNodes(workspaceRoot),
  ...
  for (const entry of entries) {
    ...
    nodes.push({
      ...
      preview: {
        kind: 'package',
        summary:
          readMarkdownHeading(markdown.body) ?? contractWorkspacePath(filePath, workspaceRoot),
      },
      metadata: {
        status: 'configured',
    ...
    nodes.push(
      createSkillNode({
        ...
        description:
          ...
          contractWorkspacePath(skillFile, workspaceRoot),
        filePath: skillFile,
        workspaceRoot,
        tags: ['skill'],
        enabled,
...
function createSkillNode(input: {
  ...
  return {
    ...
    thumbnail: {
      kind: 'color',
      label: input.sourceId === 'agent-capabilities' ? 'CMD' : 'SKL',
      accent: '#64748b',
    },
...
function projectAssetLibraryEntities(
  value: unknown,
  workspaceRoot: string,
): readonly ResourceNode[] {
  const root = assertRecord(
    value,
    contractWorkspacePath(join(workspaceRoot, ...ASSET_LIBRARY_PATH), workspaceRoot),
  );
  ...
  return entities.map((entry, index) => projectAssetLibraryEntity(entry, index));
}

function projectAssetLibraryEntity(value: unknown, index: number): ResourceNode {
...
async function readGeneratedArtifactNodes(workspaceRoot: string): Promise<readonly ResourceNode[]> {
  const artifactRoot = join(workspaceRoot, ...GENERATED_ARTIFACT_ROOT);
  if (!(await pathExists(artifactRoot))) return [];
  const filePaths = await listGeneratedArtifactFiles(artifactRoot);
  return filePaths.map((filePath) => projectGeneratedArtifact(filePath, workspaceRoot));
}

async function listGeneratedArtifactFiles(directory: string): Promise<readonly string[]> {
  const entries = await readdir(directory, { withFileTypes: true });
  const files: string[] = [];
  for (const entry of entries) {
    const filePath = join(directory, entry.name);
    if (entry.isDirectory()) {
      files.push(...(await listGeneratedArtifactFiles(filePath)));
...
function projectGeneratedArtifact(filePath: string, workspaceRoot: string): ResourceNode {
  const contractedPath = contractWorkspacePath(filePath, workspaceRoot);
  const mediaType = readGeneratedArtifactMediaType(filePath);
  return {
    id: `generated:${contractedPath}`,
    sourceId: 'generation-outputs',
    kind: 'generation',
    ...
    thumbnail: {
      kind: 'color',
      label: readGeneratedThumbnailLabel(mediaType),
      accent: '#06b6d4',
    },
    preview: {
      kind: readPreviewKind(mediaType),
      summary: contractedPath,
    },
    metadata: {
      mediaType,
      status: 'generated',
      tags: ['generated', mediaType],
    },
...
function readGeneratedArtifactMediaType(filePath: string): string {
  const extension = extname(filePath).toLowerCase();
  if (['.png', '.jpg', '.jpeg', '.webp', '.gif', '.avif', '.svg'].includes(extension)) {
    return 'image';
  }
  if (['.mp4', '.mov', '.webm', '.mkv'].includes(extension)) return 'video';
  if (['.wav', '.mp3', '.m4a', '.ogg', '.flac'].includes(extension)) return 'audio';
  return 'document';
}
...
function projectDashboardActivityEntries(
  ...
  const root = assertRecord(
    value,
    contractWorkspacePath(join(workspaceRoot, ...DASHBOARD_ACTIVITY_PATH), workspaceRoot),
  );
...
function projectDashboardActivityEntry(value: unknown, index: number): ResourceNode {
  ...
  const taskId = readRequiredString(
    entry,
    'taskId',
    `.neko/dashboard-activity.json entries[${index}]`,
  );
